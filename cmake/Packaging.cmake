...
if(NOT O3DE_PAL_TRAIT_BUILD_CPACK_SUPPORTED)
    return()
endif()

# public facing options will be used for conversion into cpack specific ones below.
set(O3DE_INSTALLER_LICENSE_URL "" CACHE STRING "Optionally embed a link to the license instead of raw text")

set(O3DE_INSTALLER_AUTO_GEN_TAG OFF CACHE BOOL
...
set(O3DE_INSTALLER_DOWNLOAD_URL "" CACHE STRING
"Base URL embedded into the installer to download additional artifacts, the host target and version \
number will automatically appended as '<version>/<host>'.  If O3DE_INSTALLER_AUTO_GEN_TAG is set, the \
...
will automatically appended as '<version>/<host>'. If O3DE_INSTALLER_AUTO_GEN_TAG is set, the full URL \
...
set(CPACK_LICENSE_URL ${O3DE_INSTALLER_LICENSE_URL})
...
set(CPACK_AUTO_GEN_TAG ${O3DE_INSTALLER_AUTO_GEN_TAG})

# attempt to apply platform specific settings
include(${O3DE_ENGINE_CMAKE_PAL_PATH}/Packaging_${O3DE_PAL_PLATFORM_WART}.cmake)
...
set(O3DE_CMAKE_PACKAGE_DOWNLOAD_PATH ${CPACK_BINARY_DIR}/${CPACK_CMAKE_PACKAGE_FILE})
...
configure_file(${O3DE_ENGINE_PATH}/cmake/Packaging/LicenseScan.cmake.in
    ${CPACK_BINARY_DIR}/LicenseScan.cmake
    @ONLY
)
o3de_install(SCRIPT ${CPACK_BINARY_DIR}/LicenseScan.cmake
    COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
)
o3de_install(FILES ${CPACK_3P_LICENSE_FILE} ${CPACK_3P_MANIFEST_FILE}
    DESTINATION .
    COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
)

configure_file(${O3DE_ENGINE_PATH}/cmake/Packaging/CMakeDownload.cmake.in
    ${CPACK_BINARY_DIR}/CMakeDownload.cmake
    @ONLY
)
o3de_install(SCRIPT ${CPACK_BINARY_DIR}/CMakeDownload.cmake
    COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
)
o3de_install(FILES ${O3DE_CMAKE_PACKAGE_DOWNLOAD_PATH}
...
set(CPACK_PRE_BUILD_SCRIPTS ${pal_dir}/PackagingPreBuild_${O3DE_PAL_HOST_PLATFORM_NAME_LOWERCASE}.cmake)
set(CPACK_POST_BUILD_SCRIPTS ${pal_dir}/PackagingPostBuild_${O3DE_PAL_HOST_PLATFORM_NAME_LOWERCASE}.cmake)
set(CPACK_CODESIGN_SCRIPT ${pal_dir}/PackagingCodeSign_${O3DE_PAL_HOST_PLATFORM_NAME_LOWERCASE}.cmake)
set(CPACK_O3DE_PYTHON_CMD ${O3DE_PYTHON_CMD})
...
# through O3DE_INSTALL_EXTERNAL_BUILD_DIRS
unset(cpack_components_contents)

set(required "FALSE")
set(disabled "FALSE")
if(${O3DE_INSTALL_PERMUTATION_COMPONENT} STREQUAL DEFAULT)
    set(required "TRUE")
else()
    set(disabled "TRUE")
endif()
string(APPEND cpack_components_contents "
list(APPEND CPACK_COMPONENTS_ALL ${O3DE_INSTALL_PERMUTATION_COMPONENT})
set(CPACK_COMPONENT_${O3DE_INSTALL_PERMUTATION_COMPONENT}_DISPLAY_NAME \"${O3DE_BUILD_PERMUTATION} common files\")
set(CPACK_COMPONENT_${O3DE_INSTALL_PERMUTATION_COMPONENT}_DESCRIPTION \"${PROJECT_NAME} scripts and common files for ${O3DE_BUILD_PERMUTATION}\")
set(CPACK_COMPONENT_${O3DE_INSTALL_PERMUTATION_COMPONENT}_DEPENDS ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME})
set(CPACK_COMPONENT_${O3DE_INSTALL_PERMUTATION_COMPONENT}_REQUIRED ${required})
set(CPACK_COMPONENT_${O3DE_INSTALL_PERMUTATION_COMPONENT}_DISABLED ${disabled})
")
...
foreach(conf IN LISTS CMAKE_CONFIGURATION_TYPES)
    ...
    set(required "FALSE")
    set(disabled "FALSE")
    if(${conf} STREQUAL profile AND ${O3DE_INSTALL_PERMUTATION_COMPONENT} STREQUAL DEFAULT)
        set(required "TRUE")
    else()
        set(disabled "TRUE")
    endif()
...
if(EXISTS \"${CMAKE_ARCHIVE_OUTPUT_DIRECTORY}/${conf}/${CMAKE_STATIC_LIBRARY_PREFIX}AzCore${CMAKE_STATIC_LIBRARY_SUFFIX}\")
    list(APPEND CPACK_COMPONENTS_ALL ${O3DE_INSTALL_PERMUTATION_COMPONENT}_${UCONF})
    set(CPACK_COMPONENT_${O3DE_INSTALL_PERMUTATION_COMPONENT}_${UCONF}_DISPLAY_NAME \"Binaries for ${O3DE_BUILD_PERMUTATION} ${conf}\")
    set(CPACK_COMPONENT_${O3DE_INSTALL_PERMUTATION_COMPONENT}_${UCONF}_DESCRIPTION \"${PROJECT_NAME} libraries and applications for ${O3DE_BUILD_PERMUTATION} ${conf}\")
    set(CPACK_COMPONENT_${O3DE_INSTALL_PERMUTATION_COMPONENT}_${UCONF}_DEPENDS ${O3DE_INSTALL_PERMUTATION_COMPONENT})
    set(CPACK_COMPONENT_${O3DE_INSTALL_PERMUTATION_COMPONENT}_${UCONF}_REQUIRED ${required})
    set(CPACK_COMPONENT_${O3DE_INSTALL_PERMUTATION_COMPONENT}_${UCONF}_DISABLED ${disabled})
endif()
")
endforeach()
...
if(O3DE_INSTALLER_DOWNLOAD_URL)
    strip_trailing_slash(${O3DE_INSTALLER_DOWNLOAD_URL} O3DE_INSTALLER_DOWNLOAD_URL)

    # this will set the following variables: CPACK_DOWNLOAD_SITE, CPACK_DOWNLOAD_ALL, and CPACK_UPLOAD_DIRECTORY (local)
    cpack_configure_downloads(
        ${O3DE_INSTALLER_DOWNLOAD_URL}
