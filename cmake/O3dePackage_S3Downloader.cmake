...
# can set O3DE_AWS_COMMAND to override the searching on your path and specify manually.
# Note:  it must be a real executable, not a script!
# see: https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html
set(O3DE_AWS_COMMAND "" CACHE FILEPATH 
    "Overrides the location of the 'aws' program, leave blank to search the path for 'aws'.  You can also use O3DE_AWS_COMMAND environment variable.")
...
# O3DE_AWS_PROFILE, which would only apply to this script,
# or, AWS_PROFILE, which would apply to other Amazon tools as well, or you can use the O3DE_AWS_PROFILE 
...
set(O3DE_AWS_PROFILE "" CACHE STRING 
        "Profile to use for 'aws' CLI.  Blank means do not use a profile.  You can also use O3DE_AWS_PROFILE or AWS_PROFILE environment variables.")

if (O3DE_AWS_PROFILE)
    o3de_set(O3DE_PACKAGE_AWS_PROFILE_COMMAND "--profile" ${O3DE_AWS_PROFILE})
elseif (DEFINED ENV{O3DE_AWS_PROFILE}) # if the CMake var isn't set (from the above cache), try the environment.
    o3de_set(O3DE_PACKAGE_AWS_PROFILE_COMMAND "--profile" $ENV{O3DE_AWS_PROFILE})
elseif (DEFINED ENV{AWS_PROFILE})
    o3de_set(O3DE_PACKAGE_AWS_PROFILE_COMMAND "--profile" $ENV{AWS_PROFILE})
endif()

# not a cache variable, but a tweakable.  We can use really short-lived credentials
# and urls because we intend to presign and then immediately fetch.
o3de_set(O3DE_PACKAGE_PRESIGN_URL_EXPIRATION_TIME 120)
...
# however, we don't want to cache the path in case the env var changes:
if (NOT O3DE_AWS_COMMAND)
    if (DEFINED ENV{O3DE_AWS_COMMAND})
        o3de_set(O3DE_AWS_COMMAND $ENV{O3DE_AWS_COMMAND})
    else()
        find_program(O3DE_AWS_COMMAND_FIND "aws")
        if (NOT "${O3DE_AWS_COMMAND_FIND}" STREQUAL "O3DE_AWS_COMMAND_FIND-NOTFOUND")
            o3de_set(O3DE_AWS_COMMAND ${O3DE_AWS_COMMAND_FIND})
        endif()
        unset(O3DE_AWS_COMMAND_FIND CACHE)
    endif()
endif()
...
function(o3de_s3_download download_url download_target output_variable_name)
    if (NOT O3DE_AWS_COMMAND)
        set(${output_variable_name} "1;Cannot find the 'aws' CLI installed, either configure its location in cmake GUI, set O3DE_AWS_COMMAND or add it to your PATH." PARENT_SCOPE)
        message(FATAL_ERROR "Unable to get a required package from an s3 bucket - Cannot find the 'aws' CLI installed, either configure its location in cmake GUI, set O3DE_AWS_COMMAND or add it to your PATH.")
        return()
    endif()
    set(COMMAND_TO_RUN ${O3DE_AWS_COMMAND} "s3" "presign" ${download_url} --expires-in ${O3DE_PACKAGE_PRESIGN_URL_EXPIRATION_TIME} ${O3DE_PACKAGE_AWS_PROFILE_COMMAND})
    ...
    if (NOT ${PRESIGN_RESULT} EQUAL 0)
        ...
        set(${output_variable_name} "1;Failed To use the AWS CLI to pre-sign: "
                                    ...
                                    "Alternatively, set O3DE_AWS_COMMAND in cmake or your environment to "
