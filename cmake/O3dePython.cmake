...
include_guard()
set(_cmake_O3dePython_cmake ${CMAKE_CURRENT_LIST_DIR})

include(${_cmake_O3dePython_cmake}/O3deSet.cmake)
include(${_cmake_O3dePython_cmake}/3rdPartyPackages.cmake)
...
set(O3DE_PAL_PYTHON_PACKAGE_FILE_NAME ${O3DE_ENGINE_PATH}/cmake/3rdParty/Platform/${O3DE_PAL_HOST_PLATFORM_NAME}/Python_${O3DE_PAL_HOST_PLATFORM_NAME_LOWERCASE}${O3DE_HOST_ARCHITECTURE_NAME_EXTENSION}.cmake)
cmake_path(NORMAL_PATH O3DE_PAL_PYTHON_PACKAGE_FILE_NAME)
if(NOT EXISTS ${O3DE_PAL_PYTHON_PACKAGE_FILE_NAME})
    # Fall back to the non-architecture-specific file
    set(O3DE_PAL_PYTHON_PACKAGE_FILE_NAME ${O3DE_ENGINE_PATH}/cmake/3rdParty/Platform/${O3DE_PAL_HOST_PLATFORM_NAME}/Python_${O3DE_PAL_HOST_PLATFORM_NAME_LOWERCASE}.cmake)
    cmake_path(NORMAL_PATH O3DE_PAL_PYTHON_PACKAGE_FILE_NAME)
endif()
include(${O3DE_PAL_PYTHON_PACKAGE_FILE_NAME})

# settings and globals
o3de_set(O3DE_PYTHON_DEFAULT_REQUIREMENTS_TXT "${O3DE_ENGINE_PATH}/python/requirements.txt")
...
execute_process(COMMAND ${CMAKE_COMMAND} -P ${O3DE_ENGINE_PATH}/cmake/CalculateEnginePathId.cmake "${O3DE_ENGINE_PATH}/"
...
o3de_set(O3DE_PYTHON_VENV_PATH ${PYTHON_VENV_PATH})

function(o3de_setup_python_venv)
    ...
    # match the current package hash ${O3DE_PYTHON_PACKAGE_HASH} then reset the venv and request
    # that a new venv is created with the current Python package at :
    # 
    if (EXISTS "${PYTHON_VENV_PATH}/.hash")
        file(READ "${PYTHON_VENV_PATH}/.hash" O3DE_CURRENT_VENV_PACKAGE_HASH
             LIMIT 80)
        if (NOT ${O3DE_CURRENT_VENV_PACKAGE_HASH} STREQUAL ${O3DE_PYTHON_PACKAGE_HASH})
            ...
            execute_process(COMMAND ${PYTHON_VENV_PATH}/${O3DE_PYTHON_VENV_PYTHON} --version
                            OUTPUT_QUIET
    ...
    if (CREATE_NEW_VENV)
        ...
        execute_process(COMMAND "${PYTHON_PACKAGES_ROOT_PATH}/${O3DE_PYTHON_PACKAGE_NAME}/${O3DE_PYTHON_BIN_PATH}/${O3DE_PYTHON_EXECUTABLE}" -m venv "${PYTHON_VENV_PATH}" --without-pip --clear
                        WORKING_DIRECTORY "${PYTHON_PACKAGES_ROOT_PATH}/${O3DE_PYTHON_PACKAGE_NAME}/${O3DE_PYTHON_BIN_PATH}"
                        COMMAND_ECHO STDOUT
                        RESULT_VARIABLE command_result)
        ...
        o3de_post_python_venv_install(${PYTHON_VENV_PATH})

        # Manually install pip into the virtual environment
        message(STATUS "Installing pip into venv at ${PYTHON_VENV_PATH} (${PYTHON_VENV_PATH}/${O3DE_PYTHON_BIN_PATH})")

        execute_process(COMMAND "${PYTHON_VENV_PATH}/${O3DE_PYTHON_VENV_PYTHON}" -m ensurepip --upgrade --default-pip -v
                        WORKING_DIRECTORY "${PYTHON_VENV_PATH}/${O3DE_PYTHON_VENV_BIN_PATH}"
                        COMMAND_ECHO STDOUT
                        RESULT_VARIABLE command_result)

        if (NOT ${command_result} EQUAL 0)
            message(FATAL_ERROR "Error installing pip into venv: ${O3DE_PIP_ERROR}")
        endif()

        file(WRITE "${PYTHON_VENV_PATH}/.hash" ${O3DE_PYTHON_PACKAGE_HASH})
    endif()
...
function(update_pip_requirements requirements_file_path unique_name)
    ...
    set(stamp_file ${O3DE_PYTHON_VENV_PATH}/requirements_files/${unique_name}.stamp)
    ...
    if(EXISTS ${stamp_file} AND ${stamp_file} IS_NEWER_THAN ${requirements_file_path})
        ...
        o3de_package_is_newer_than(${O3DE_PYTHON_PACKAGE_NAME} ${stamp_file} package_is_newer)
    ...
    execute_process(COMMAND 
        ${O3DE_PYTHON_CMD} -m pip install -r "${requirements_file_path}" --disable-pip-version-check --no-warn-script-location
        WORKING_DIRECTORY ${O3DE_ENGINE_PATH}/python
...
function(o3de_pip_install_local_package_editable package_folder_path pip_package_name)
    set(stamp_file ${O3DE_PYTHON_VENV_PATH}/packages/pip_installs/${pip_package_name}.stamp)
    get_filename_component(stamp_file_directory ${stamp_file} DIRECTORY)
    file(MAKE_DIRECTORY ${stamp_file_directory})
    ...
    execute_process(COMMAND 
        ${O3DE_PYTHON_CMD} -m pip uninstall ${pip_package_name} -y  --disable-pip-version-check 
        WORKING_DIRECTORY ${O3DE_ENGINE_PATH}/python
        RESULT_VARIABLE PIP_RESULT
        OUTPUT_VARIABLE PIP_OUT 
        ERROR_VARIABLE PIP_OUT
    ...
    execute_process(COMMAND 
        ${O3DE_PYTHON_CMD} -m pip install -e ${package_folder_path} --no-deps --disable-pip-version-check  --no-warn-script-location 
        WORKING_DIRECTORY ${O3DE_ENGINE_PATH}/python
        RESULT_VARIABLE PIP_RESULT
        OUTPUT_VARIABLE PIP_OUT 
        ERROR_VARIABLE PIP_OUT
...
o3de_associate_package(PACKAGE_NAME ${O3DE_PYTHON_PACKAGE_NAME} TARGETS "Python" PACKAGE_HASH ${O3DE_PYTHON_PACKAGE_HASH})
o3de_set_package_download_location(${O3DE_PYTHON_PACKAGE_NAME} ${PYTHON_PACKAGES_ROOT_PATH})
o3de_set_package_download_cache_location(${O3DE_PYTHON_PACKAGE_NAME} ${PYTHON_PACKAGE_CACHE_ROOT_PATH})
o3de_download_associated_package(Python)
o3de_setup_python_venv()

if (NOT CMAKE_SCRIPT_MODE_FILE)


    # note - if you want to use a normal python via FindPython instead of the O3DE package above,
    ...
    find_package(Python ${O3DE_PYTHON_VERSION} REQUIRED)
    ...
    if (NOT EXISTS "${PYTHON_VENV_PATH}/" OR NOT Python_EXECUTABLE OR NOT Python_HOME OR NOT Python_PATHS)
        ...
        LIST(APPEND CMAKE_PROGRAM_PATH "${O3DE_ENGINE_PATH}/python")

        # those using python should call it via O3DE_PYTHON_CMD - it adds the extra "-s" param
        ...
        if (${CMAKE_HOST_SYSTEM_NAME} STREQUAL "Windows")
            set(O3DE_PYTHON_CMD "${O3DE_ENGINE_PATH}/python/python.cmd" "-s")
        else()
            set(O3DE_PYTHON_CMD "${O3DE_ENGINE_PATH}/python/python.sh" "-s")
        endif()

        update_pip_requirements(${O3DE_PYTHON_DEFAULT_REQUIREMENTS_TXT} default_requirements)
        ...
        if(O3DE_PAL_TRAIT_BUILD_TESTS_SUPPORTED AND NOT INSTALLED_ENGINE)
            o3de_pip_install_local_package_editable(${O3DE_ENGINE_PATH}/Tools/O3deTestTools o3de-test-tools)
            o3de_pip_install_local_package_editable(${O3DE_ENGINE_PATH}/Tools/RemoteConsole/o3de_remote_console o3de-remote-console)
            # EditorPythonTestTools lives inside the AutomatedTesting
            # project, which is not part of the engine tree in composed
            # workspaces — only install when present
            if(EXISTS ${O3DE_ENGINE_PATH}/AutomatedTesting/Gem/PythonTests/EditorPythonTestTools/setup.py)
                o3de_pip_install_local_package_editable(${O3DE_ENGINE_PATH}/AutomatedTesting/Gem/PythonTests/EditorPythonTestTools editor-python-test-tools)
            endif()
        endif()

        o3de_pip_install_local_package_editable(${O3DE_ENGINE_PATH}/scripts/o3de o3de)
    endif()
endif()
