...
set(O3DE_TEST_IMPACT_CONSOLE_NATIVE_STATIC_TARGET "TestImpact.Frontend.Console.Native.Static")

# Name of test impact framework python coverage gem target
set(O3DE_TEST_IMPACT_PYTHON_COVERAGE_STATIC_TARGET "PythonCoverage.Editor.Static")

# Name of test impact framework native console target
set(O3DE_TEST_IMPACT_NATIVE_CONSOLE_TARGET "TestImpact.Frontend.Console.Native")

# Name of test impact framework native console target
set(O3DE_TEST_IMPACT_PYTHON_CONSOLE_TARGET "TestImpact.Frontend.Console.Python")

# Directory for test impact artifacts and data
set(O3DE_TEST_IMPACT_WORKING_DIR "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/TestImpactFramework")

# Directory name for TIAF temp directory
set(O3DE_TEST_IMPACT_TEMP_DIR "Temp")

# Directory for artifacts generated at runtime 
set(O3DE_TEST_IMPACT_RUNTIME_TEMP_DIR "${O3DE_TEST_IMPACT_WORKING_DIR}/$<CONFIG>/${O3DE_TEST_IMPACT_TEMP_DIR}")

# Directory name for TIAF persistent directory
set(O3DE_TEST_IMPACT_PERSISTENT_DIR "Persistent")

# Directory for files that persist between runtime runs
set(O3DE_TEST_IMPACT_RUNTIME_PERSISTENT_DIR "${O3DE_TEST_IMPACT_WORKING_DIR}/$<CONFIG>/${O3DE_TEST_IMPACT_PERSISTENT_DIR}")

# Directory for static artifacts produced as part of the build system generation process
set(O3DE_TEST_IMPACT_ARTIFACT_DIR "${O3DE_TEST_IMPACT_WORKING_DIR}/Artifact")

# Directory for source to build target mappings
set(O3DE_TEST_IMPACT_SOURCE_TARGET_MAPPING_DIR "${O3DE_TEST_IMPACT_ARTIFACT_DIR}/Mapping")

# Directory for build target dependency/depender graphs
set(O3DE_TEST_IMPACT_TARGET_DEPENDENCY_DIR "${O3DE_TEST_IMPACT_ARTIFACT_DIR}/Dependency")

# Main test enumeration file for all test types
set(O3DE_TEST_IMPACT_TEST_TYPE_FILE "${O3DE_TEST_IMPACT_ARTIFACT_DIR}/TestType/All.tests")

# Main gem target file for all shared library gems
set(O3DE_TEST_IMPACT_GEM_TARGET_FILE "${O3DE_TEST_IMPACT_ARTIFACT_DIR}/BuildType/All.gems")

# File name for TIAF config files.
set(O3DE_TEST_IMPACT_CONFIG_FILE_NAME "tiaf.json")

# Path to the config file for each build configuration
set(O3DE_TEST_IMPACT_CONFIG_FILE_PATH "${O3DE_TEST_IMPACT_RUNTIME_PERSISTENT_DIR}/${O3DE_TEST_IMPACT_CONFIG_FILE_NAME}")

# Preprocessor directive for the config file path
set(O3DE_TEST_IMPACT_CONFIG_FILE_PATH_DEFINITION "O3DE_TEST_IMPACT_DEFAULT_CONFIG_FILE=\"${O3DE_TEST_IMPACT_CONFIG_FILE_PATH}\"")

# Path to file used to store data required by TIAF tests
set(O3DE_TEST_IMPACT_PYTEST_FILE_PATH "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/$<CONFIG>")

# Path to the directory that the result of native runs will be stored in.
set(O3DE_TEST_IMPACT_NATIVE_TEST_RUN_DIR "${GTEST_XML_OUTPUT_DIR}")

# Path to the directory that the result of python runs will be stored in.
set(O3DE_TEST_IMPACT_PYTHON_TEST_RUN_DIR "${PYTEST_XML_OUTPUT_DIR}")

#! o3de_test_impact_rebase_file_to_repo_root: rebases the relative and/or absolute path to be relative to repo root directory and places the resulting path in quotes.
#
...
function(o3de_test_impact_rebase_file_to_repo_root INPUT_FILE OUTPUT_FILE RELATIVE_DIR_ABS)
    ...
    file(RELATIVE_PATH rebased_file ${O3DE_ENGINE_PATH} ${rebased_file})
    set(${OUTPUT_FILE} ${rebased_file} PARENT_SCOPE)
endfunction()

#! o3de_test_impact_rebase_files_to_repo_root: rebases the relative and/or absolute paths to be relative to repo root directory and places the resulting paths in quotes.
#
...
function(o3de_test_impact_rebase_files_to_repo_root INPUT_FILES OUTPUT_FILES RELATIVE_DIR_ABS)
    # Rebase all paths in list to repo root
    set(rebased_files "")
    foreach(in_file IN LISTS INPUT_FILES)
        o3de_test_impact_rebase_file_to_repo_root(
            ${in_file}
            out_file
            ${RELATIVE_DIR_ABS}
        )
...
#! o3de_test_impact_get_test_launch_method: gets the launch method (either standalone or testrunner) for the specified target.
#
# \arg:TARGET_NAME name of the target
# \arg:LAUNCH_METHOD the type string for the specified target
function(o3de_test_impact_get_test_launch_method TARGET_NAME LAUNCH_METHOD)
...
#! o3de_test_impact_extract_google_test_name: extracts the google test name from the composite 'namespace::test_name' string
#
# \arg:COMPOSITE_TEST test in the form 'namespace::test'
# \arg:TEST_NAME name of test
function(o3de_test_impact_extract_google_test COMPOSITE_TEST TEST_NAMESPACE TEST_NAME)
    get_property(test_components GLOBAL PROPERTY O3DE_ALL_TESTS_${COMPOSITE_TEST}_TEST_NAME)
    ...
    if(num_test_components LESS 2)
        message(FATAL_ERROR "The test ${test_components} appears to have been specified without a namespace, i.e.:\o3de_add_googletest/benchmark(NAME ${test_components})\nInstead of (perhaps):\o3de_add_googletest/benchmark(NAME Gem::${test_components})\nPlease add the missing namespace before proceeding.")
    endif()
...
#! o3de_test_impact_extract_python_test_name: extracts the python test name from the composite 'namespace::test_name' string
#
# \arg:COMPOSITE_TEST test in form 'namespace::test' or 'test'
# \arg:TEST_NAME name of test
function(o3de_test_impact_extract_python_test COMPOSITE_TEST TEST_NAME)
    get_property(test_components GLOBAL PROPERTY O3DE_ALL_TESTS_${COMPOSITE_TEST}_TEST_NAME)
...
#! o3de_test_impact_extract_google_test_params: extracts the suites for the given google test.
#
# \arg:COMPOSITE_TEST test in the form 'namespace::test'
...
function(o3de_test_impact_extract_google_test_params COMPOSITE_TEST COMPOSITE_SUITES TEST_NAMESPACE TEST_NAME TEST_SUITES)
...
#! o3de_test_impact_extract_python_test_params: extracts the python test name and relative script path parameters.
#
# \arg:COMPOSITE_TEST test in form 'namespace::test' or 'test'
# \arg:COMPOSITE_SUITES composite list of suites for this target
# \arg:TEST_NAMESPACE namespace of test
# \arg:TEST_NAME name of test
# \arg:TEST_SUITES extracted list of suites for this target in JSON format
function(o3de_test_impact_extract_python_test_params COMPOSITE_TEST COMPOSITE_SUITES TEST_NAMESPACE TEST_NAME TEST_SUITES)
    get_property(script_path GLOBAL PROPERTY O3DE_ALL_TESTS_${COMPOSITE_TEST}_SCRIPT_PATH)
    get_property(test_command GLOBAL PROPERTY O3DE_ALL_TESTS_${COMPOSITE_TEST}_TEST_COMMAND)
...
#! o3de_test_impact_write_test_enumeration_file: exports the main test list to file.
# 
# \arg:TEST_ENUMERATION_TEMPLATE_FILE path to test enumeration template file
function(o3de_test_impact_write_test_enumeration_file TEST_ENUMERATION_TEMPLATE_FILE)
    get_property(O3DE_ALL_TESTS GLOBAL PROPERTY O3DE_ALL_TESTS)
    ...
    # Walk the test list
    foreach(test ${O3DE_ALL_TESTS})
        message(TRACE "Parsing ${test}")
        get_property(test_params GLOBAL PROPERTY O3DE_ALL_TESTS_${test}_PARAMS)
        get_property(test_type GLOBAL PROPERTY O3DE_ALL_TESTS_${test}_TEST_LIBRARY)

        if("${test_type}" STREQUAL "pytest")
            # Python tests
            o3de_test_impact_extract_python_test_params(${test} "${test_params}" test_namespace test_name test_suites)
        ...
        elseif("${test_type}" STREQUAL "googletest")
            # Google tests
            o3de_test_impact_extract_google_test_params(${test} "${test_params}" test_namespace test_name test_suites)
            o3de_test_impact_get_test_launch_method(${test} launch_method)
    ...
    configure_file(${TEST_ENUMERATION_TEMPLATE_FILE} ${O3DE_TEST_IMPACT_TEST_TYPE_FILE})
endfunction()

#! o3de_test_impact_write_gem_target_enumeration_file: exports the main gem target list to file.
#
# \arg:GEM_TARGET_TEMPLATE_FILE path to source to gem target template file
function(o3de_test_impact_write_gem_target_enumeration_file GEM_TARGET_TEMPLATE_FILE)
    get_property(O3DE_ALL_TARGETS GLOBAL PROPERTY O3DE_ALL_TARGETS)

    set(enumerated_gem_targets "")
    # Walk the build targets
    foreach(aliased_target ${O3DE_ALL_TARGETS})
    ...
    set(mapping_path "${O3DE_TEST_IMPACT_GEM_TARGET_FILE}")
    configure_file(${GEM_TARGET_TEMPLATE_FILE} ${mapping_path})
endfunction()

#! o3de_extract_aliased_target_dependencies: recursively extracts the aliases of a target to retrieve the true de-aliased target.
#
# \arg:TARGET target to de-alias
function(o3de_extract_aliased_target_dependencies TARGET DE_ALIASED_TARGETS)
    if(NOT ARGN)
        # Entry point of recursive call, set the parent target and clear any existing aliases
        set(PARENT_TARGET ${TARGET})
        set_property(GLOBAL PROPERTY O3DE_EXTRACT_ALIASED_TARGET_DEPENDENCIES_DE_ALIASED_TARGETS_${PARENT_TARGET} "")
    endif()
    ...
    if(${aliased_targets})
        ...
        foreach(aliased_target ${aliased_targets})
            ...
            o3de_extract_aliased_target_dependencies(${aliased_target} empty ${PARENT_TARGET})
        endforeach()
    else()
        # No more aliases found for this target, add this target as an alias for the parent target
        set_property(GLOBAL APPEND PROPERTY O3DE_EXTRACT_ALIASED_TARGET_DEPENDENCIES_DE_ALIASED_TARGETS_${PARENT_TARGET} ${TARGET})
    endif()

    if(NOT ARGN)
        # Exit point of recursive call
        get_property(de_aliased_targets GLOBAL PROPERTY O3DE_EXTRACT_ALIASED_TARGET_DEPENDENCIES_DE_ALIASED_TARGETS_${TARGET})
        set(${DE_ALIASED_TARGETS} ${de_aliased_targets} PARENT_SCOPE)
    endif()
endfunction()

#! o3de_extract_target_dependencies: extracts the target dependencies for the specified target as a comma separated list.
function(o3de_extract_target_dependencies INPUT_DEPENDENCY_LIST OUTPUT_DEPENDENCY_LIST)
    set(dependencies "")
    ...
    foreach(qualified_target_name ${INPUT_DEPENDENCY_LIST})
        ...
        if(TARGET ${qualified_target_name})
            ...
            if(NOT is_imported OR is_gem_module)
                ...
                o3de_extract_aliased_target_dependencies(${target_to_add} de_aliased_targets)
...
#! o3de_get_target_type: retrieves the type of target (either production or test target).
#
# \arg:TARGET target to get the type for
# \arg:TARGET_TYPE the retrieved target type
function(o3de_get_target_type TARGET TARGET_TYPE)
...
#! o3de_test_impact_export_source_target_mappings: exports the static source to target mappings to file.
#
# \arg:MAPPING_TEMPLATE_FILE path to source to target template file
function(o3de_test_impact_export_source_target_mappings MAPPING_TEMPLATE_FILE)
    get_property(O3DE_ALL_TARGETS GLOBAL PROPERTY O3DE_ALL_TARGETS)
    ...
    foreach(aliased_target ${O3DE_ALL_TARGETS})
        ...
        file(RELATIVE_PATH target_path ${O3DE_ENGINE_PATH} ${target_path_abs})

        # Target type
        o3de_get_target_type(${target_name} target_type)
        ...
        # Dependencies
        get_property(build_dependencies GLOBAL PROPERTY O3DE_ALL_TARGETS_${target}_BUILD_DEPENDENCIES)
        get_property(runtime_dependencies GLOBAL PROPERTY O3DE_ALL_TARGETS_${target}_RUNTIME_DEPENDENCIES)
        o3de_extract_target_dependencies("${build_dependencies}" build_dependencies)
        o3de_extract_target_dependencies("${runtime_dependencies}" runtime_dependencies)
        ...
        if(DEFINED autogen_input_files AND autogen_output_files)
            ...
            o3de_test_impact_rebase_files_to_repo_root(
                "${autogen_input_files}"
                autogen_input_files
                ${target_path_abs}
            )
        ...
        # Write out source to target mapping file
        set(mapping_path "${O3DE_TEST_IMPACT_SOURCE_TARGET_MAPPING_DIR}/${target}.target")
        configure_file(${MAPPING_TEMPLATE_FILE} ${mapping_path})
    endforeach()
endfunction()

#! o3de_test_impact_write_config_file: writes out the test impact framework config file using the data derived from the build generation process.
# 
# \arg:CONFIG_TEMPLATE_FILE path to the runtime configuration template file
# \arg:BIN_DIR path to repo binary output directory
function(o3de_test_impact_write_config_file CONFIG_TEMPLATE_FILE BIN_DIR)
    # Platform this config file is being generated for
    set(platform ${O3DE_PAL_PLATFORM_NAME})
    ...
    # Python command
    set(python_cmd "${O3DE_ENGINE_PATH}/python/python.cmd")

    # Repository root
    set(repo_dir ${O3DE_ENGINE_PATH})
    
    # Test impact framework output binary dir
    set(bin_dir ${BIN_DIR})
    
    # Native temp dir
    set(native_temp_dir "${O3DE_TEST_IMPACT_RUNTIME_TEMP_DIR}/Native")

    # Native active persistent data dir
    set(native_active_dir "${O3DE_TEST_IMPACT_RUNTIME_PERSISTENT_DIR}/Native/active")

    # Native historic persistent data dir
    set(native_historic_dir "${O3DE_TEST_IMPACT_RUNTIME_PERSISTENT_DIR}/Native/historic")

    # Python temp dir
    set(python_temp_dir "${O3DE_TEST_IMPACT_RUNTIME_TEMP_DIR}/Python")

    # Python active persistent data dir
    set(python_active_dir "${O3DE_TEST_IMPACT_RUNTIME_PERSISTENT_DIR}/Python/active")

    # Python historic persistent data dir
    set(python_historic_dir "${O3DE_TEST_IMPACT_RUNTIME_PERSISTENT_DIR}/Python/historic")

    # Source to target mappings dir
    set(source_target_mapping_dir "${O3DE_TEST_IMPACT_SOURCE_TARGET_MAPPING_DIR}")
    
    # Test type artifact file
    set(test_target_type_file "${O3DE_TEST_IMPACT_TEST_TYPE_FILE}")

    # Gem target file
    set(gem_target_file "${O3DE_TEST_IMPACT_GEM_TARGET_FILE}")
    
    # Build dependency artifact dir
    set(target_dependency_dir "${O3DE_TEST_IMPACT_TARGET_DEPENDENCY_DIR}")

    # Test impact analysis framework native runtime binary
    if(O3DE_TEST_IMPACT_NATIVE_TEST_TARGETS_ENABLED)
        set(native_runtime_bin "$<TARGET_FILE:${O3DE_TEST_IMPACT_NATIVE_CONSOLE_TARGET}>")
    endif()

    # Test impact analysis framework python runtime binary
    if(O3DE_TEST_IMPACT_PYTHON_TEST_TARGETS_ENABLED)
        set(python_runtime_bin "$<TARGET_FILE:${O3DE_TEST_IMPACT_PYTHON_CONSOLE_TARGET}>")
    endif()
    
    # Substitute config file template with above vars
    o3de_file_read("${CONFIG_TEMPLATE_FILE}" config_file)
    ...
    file(GENERATE
        OUTPUT "${O3DE_TEST_IMPACT_CONFIG_FILE_PATH}" 
        CONTENT "${config_file}"
    )
...
#! o3de_test_impact_write_pytest_file: writes out the test information utilised by our TIAF testing tools, using the data derived from the build generation process.
# 
# \arg:CONFIGURATION_FILE path to the test data template file
function(o3de_test_impact_write_pytest_file CONFIGURATION_FILE)

    # For each configuration type, compile the build info we need and add it to our array
    set(build_configs "")
    foreach(config_type ${O3DE_CONFIGURATION_TYPES})
        set(config_path "${O3DE_TEST_IMPACT_WORKING_DIR}/${config_type}/${O3DE_TEST_IMPACT_PERSISTENT_DIR}/${O3DE_TEST_IMPACT_CONFIG_FILE_NAME}")
    ...
    o3de_file_read("${CONFIGURATION_FILE}" test_file)
    string(CONFIGURE ${test_file} test_file)
    file(GENERATE
        OUTPUT "${O3DE_TEST_IMPACT_PYTEST_FILE_PATH}/o3de_test_impact_test_data.json"
        CONTENT "${test_file}"
    )

endfunction()

#! o3de_test_impact_clean_directories: Removes the artifact directory, test output directory, temp directory
#! and the persistent directories containing TIAF configs for each build configuration.
function(o3de_test_impact_clean_directories)

    # Clean the output folders of native and python tests to ensure only the most current run is in there.
    file(REMOVE_RECURSE ${O3DE_TEST_IMPACT_NATIVE_TEST_RUN_DIR})
    file(REMOVE_RECURSE ${O3DE_TEST_IMPACT_PYTHON_TEST_RUN_DIR})

    # For each build configuration type, delete the persistent and temp folders
    foreach(config_type ${O3DE_CONFIGURATION_TYPES})
        file(REMOVE_RECURSE "${O3DE_TEST_IMPACT_WORKING_DIR}/${config_type}/${O3DE_TEST_IMPACT_PERSISTENT_DIR}")
        file(REMOVE_RECURSE "${O3DE_TEST_IMPACT_WORKING_DIR}/${config_type}/${O3DE_TEST_IMPACT_TEMP_DIR}")
    endforeach()
    ...
    file(REMOVE_RECURSE "${O3DE_TEST_IMPACT_ARTIFACT_DIR}")
endfunction()

#! o3de_test_impact_post_step: runs the post steps to be executed after all other cmake scripts have been executed.
function(o3de_test_impact_post_step)
    ...
    o3de_test_impact_clean_directories()
    ...
    o3de_test_impact_export_source_target_mappings(
        "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/SourceToTargetMapping.in"
    )

    # Export the enumerated tests
    o3de_test_impact_write_test_enumeration_file(
        "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/EnumeratedTests.in"
    )

    # Export the enumerated gems
    o3de_test_impact_write_gem_target_enumeration_file(
        "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/EnumeratedGemTargets.in"
    )

    # Write out the configuration file
    o3de_test_impact_write_config_file(
        "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/ConsoleFrontendConfig.in"
        ${bin_dir}
    )

    # Write out required test data into config file.
    o3de_test_impact_write_pytest_file(
        "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/O3deTestImpactTestData.in"
    )
    ...
    file(COPY "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/CMakeGraphVizOptions.cmake" DESTINATION ${CMAKE_BINARY_DIR})
endfunction()
