...
# Base directory where all country folders will be created
base_dir = "F:\\github\\byrcolin\\canonical.o3de.org\\src\\countries"

# Template for repo.json files
def create_repo_json(country, repo_type):
    """Create a customized repo.json for a specific country and repo type"""
    return {
        "$schema": "https://canonical.o3de.org/o3de-repo-2.0.0.json",
        "$schemaVersion": "2.0.0",
        "repo": {
            "name": f"org.o3de.repo.{country['alpha_2'].lower()}.{repo_type}",
            "uri": f"https://canonical.o3de.org/countries/{country['alpha_2'].lower()}/{repo_type}/repo.json",
            "type": f"{repo_type}"
        },
        "version": "1.0.0",
        "display_name": f"{repo_type.capitalize()} {country['name']}.",
        "summary": f"The Open 3D Engine's {repo_type.capitalize()} Objects for {country['name']}.",
        "additional_info": f"These objects are {repo_type} by the O3DE team for {country['name']}.",
        ...
        "remote": {
            ...
            "repos": [
                "https://canonical.o3de.org/curated/repo.json" if repo_type == "curated" else "https://canonical.o3de.org/uncurated/repo.json"
            ],
            "restricteds": [
            ]
        },
        "source_control": {
            "gitUri": "https://github.com/o3de/canonical.o3de.org.git",
            "relative_path": f"countries/{country['alpha_2'].lower()}/{repo_type}"
        }
    }
...
def _run_createdirs(args) -> int:
    ...
    for country in countries:
        ...
        encoded_country = country["alpha_2"]
        country_path = os.path.join(base_dir, encoded_country)
        
        # Create directories for this country
        for repo_type in ["curated", "uncurated"]:
            repo_path = os.path.join(country_path, repo_type)
            os.makedirs(repo_path, exist_ok=True)
            
            # Create repo.json for this country and repo type
            repo_json = create_repo_json(country, repo_type)
            
            # Write to file
            repo_file_path = os.path.join(repo_path, "repo.json")
            with open(repo_file_path, "w", encoding="utf-8") as f:
                json.dump(repo_json, f, indent=4, ensure_ascii=False)
...
def add_args(subparsers) -> None:
    ...
    call add_args and execute: python o3de.py createdirs
    :param subparsers: the caller instantiates subparsers and passes it in here
    """
    createdirs_subparser = subparsers.add_parser('createdirs')

    createdirs_subparser.description = "Creates directories and repo.json files for each country"
                       
    createdirs_subparser.set_defaults(func=_run_createdirs)