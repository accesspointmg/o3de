...
#
"""
This file contains all the code that has to do with updating the SPDX schema and ISO 3166 countries
"""
...
from o3de import o3de_object, repo, utils, validation, compatibility, cmake
...
def licenses_to_schema(licenses_json_data, current_patterns_schema_json_data, output_json_path: pathlib.Path = None) -> int:
    """
    Updates a SPDX license json into a schema used by the schema 2.0.0   
    ...
    new_license_schema = {
        ...
        "properties": {
            "license_identifier": {
                "type": "string",
                "description": "The SPDX identifier for the license. i.e. 'Apache-2.0' or the custom license identifier.",
                "oneOf": [
                    {
                        "enum": [
            ...
            "display_name": {
                "type": "string",
                "description": "The human readable name of the license. i.e. 'Apache License 2.0'"
            },                
            "uri": {
                "description": "URI of the license. i.e. 'https://spdx.org/licenses/Apache-2.0.html'",
                "$ref": "./o3de-patterns-2.0.0.json#/definitions/httpsftpsUri"
            },
            "relative_path": {
                "definition": "It is best practice to include a copy of the license in the object, this is the relative path to the license file from this object's root. i.e. 'licenses/mylicense.txt'",
                "$ref": "./o3de-patterns-2.0.0.json#/definitions/relativePath"
            },
            "scopes": {
                "type" : "array",
                "items" : {
                    "type": "string",
                    "description": "If this license is limited to a specific scope state that here. i.e. 'Applies only to non-commercial uses.'"
                }
            },
            "files" : {
                "description" : "Specific files referenced for this license may be enumerated here. This is often used when license information is not written into the file itself.",
                "type" : "array",
                "items" : {
                    "type" : "object",
                    "properties" : {
                        "relative_path" : {
                            "description" : "The relative path of the file from the root.",
                            "$ref": "./o3de-patterns-2.0.0.json#/definitions/relativePath"
                        },
                        "attributions" : {
                            "type" : "array",
                            "items" : {
                                "description" : "This field provides a place to record acknowledgements that may be required to be communicated in some contexts. This is not meant to include the actual complete license text, and may or may not include copyright notices (see also copyrightText). Use this field to record other acknowledgements, such as particular clauses from license texts, which may be necessary or desirable to reproduce.",
                                "type" : "string"
                            }
                        },
                        "checksum" : {
                            "description" : "The checksum property provides a mechanism that can be used to verify that the contents of a file have not changed.",
                            "$ref": "./o3de-patterns-2.0.0.json#/definitions/sha256"
                        },
                        "comments" : {
                            "description" : "This field provides a place to record comments about the file which may be desirable to reproduce.",
                            "type" : "array",
                            "items" : {
                                "type" : "string"
                            }
                        ...
                        "copyright" : {
                            "description" : "The text of copyright declarations for the file. If the copyrightText field is not present, it implies an equivalent meaning to NOASSERTION.",
                            "type" : "string"
                        },                      
                        "notice" : {
                            "description" : "This field provides a place to record potential legal notices found in the file. This may or may not include copyright statements.",
                            "type" : "string"
                        }
                    },
                    "required" : [
    ...
    if 'licenses' in licenses_json_data:
        ...
        new_license_schema['properties']['license_identifier']['oneOf'][0]['enum'] = [
        ...
        for license in licenses_json_data['licenses']:
            condition = {
                "if": {
                    "properties": {
                        "license_identifier": {"const": license['licenseId']}
                    }
                },
                "then": {
                    "properties": {
                        "display_name": {"const": license['name']},
                        "uri": {"const": license['reference']}
                    }
                }
            }
...
def countries_to_schema(countries_json_data, current_patterns_schema_json_data, output_json_path: pathlib.Path = None) -> int:
    """
    Updates a countries json into a schema used by the schema 2.0.0   
    ...
    new_country_schema = {
        ...
        "properties": {
            "code": {
                "type": "string",
                "description": "The Two letter country code. i.e. 'US' or 'AF'.",
                "pattern": "^[A-Z]{2}$",
                "default": "US",
                "oneOf": [
                    {
                        "enum": [
                            # we read the countries.json and read each of the country codes into this enum
                        ]
                    }             
                ]
            },
            "name": {
                "type": "string",
                "description": "The name of the country. i.e. 'United States' or 'Afghanistan'."
            },
            "official_name": {
                "type": "string",
                "description": "The official name of the country. i.e. 'United States of America' or 'Islamic Republic of Afghanistan'."
            }
        },
        "required": [
            "code"
        ...
        "allOf": [
            # This we will populate with conditions for each country
        ]
    }

    # Extract country codes for enum
    new_country_schema['properties']['code']['oneOf'][0]['enum'] = [
        country['alpha_2'] for country in countries_json_data
    ]

    # Build conditions for each country
    for country in countries_json_data:
        condition = {
            "if": {
                "properties": {
                    "code": {
                        "const": country['alpha_2']
                    }
                }
            },
            "then": {
                "properties": {
                    ...
                    "official_name": {
                        "const": country.get('official_name', country['name'])}
                }
            }
        }
        new_country_schema['allOf'].append(condition)

    #replace the country schema in the current patterns schema json
    current_patterns_schema_json_data['definitions']['country'] = new_country_schema
    ...
    if isinstance(output_json_path, pathlib.PurePath):
        ...
        try:
            with open(countries_json_data, 'w') as f:
                json.dump(current_patterns_schema_json_data, f, indent=4)
        except Exception as e:
            logger.error(f'Failed to write json file: {countries_json_data} {e}')
            return 1
...
def _run_license_to_schema(args: argparse) -> int:
    ...
    # if no output file is specified, transform fully inplace
    else:
...
def _run_countries_to_schema(args: argparse) -> int:
    ...
    #load countries.json
    countries_json_data = {}
    if hasattr(args, 'countries_json') and args.countries_json:
        # Load the countries.json file
        try:
            with open(args.countries_json, 'r') as f:
                countries_json_data = json.load(f)
        except Exception as e:
            logger.error(f'Failed to load json file: {args.countries_json} {e}')
            return 1
    else:
        countries_json_data = countries
    
    if hasattr(args, 'output_patterns_schema_json') and args.output_patterns_schema_json:
        return countries_to_schema(countries_json_data, current_patterns_schema_json_data, output_json_path=args.output_patterns_schema_json)

    # if no output file is specified, transform fully inplace
    else:
        return countries_to_schema(countries_json_data, current_patterns_schema_json_data, output_json_path=args.current_patterns_schema_json)
...
def add_args(subparsers) -> None:
    ...
    call add_args and execute: 
        python o3de.py license-to-schema --license-json "C:/license.json" --output-patterns-schema-json "C:/schemas/license.json"
        python o3de.py countries-to-schema --countries-json "C:/countries.json" --output-patterns-schema-json "C:/schemas/countries.json"
    :param subparsers: the caller instantiates subparsers and passes it in here
    """
    license_to_schema_subparser = subparsers.add_parser('license-to-schema')
    utils.add_verbosity_arg(license_to_schema_subparser)
    
    license_to_schema_subparser.description = "Upgrades O3DE JSON schema file to newer license versions"
                           
    license_to_schema_subparser.add_argument('--current-patterns-schema-json', 
                      dest='current_patterns_schema_json',
                      type=pathlib.Path,
                      required=True,
                      help='The path to the current patterns schema json file.')
    
    license_to_schema_subparser.add_argument('--licenses-json', 
                       dest='licenses_json',
                       type=pathlib.Path, 
                       required=False,
    ...
    license_to_schema_subparser.add_argument('--output-patterns-schema-json', 
    ...
    license_to_schema_subparser.set_defaults(func=_run_license_to_schema)
    

    countries_to_schema_subparser = subparsers.add_parser('countries-to-schema')
    utils.add_verbosity_arg(countries_to_schema_subparser)

    countries_to_schema_subparser.description = "Upgrades O3DE JSON schema files to newer countries"
                           
    countries_to_schema_subparser.add_argument('--current-patterns-schema-json', 
                      dest='current_patterns_schema_json',
                      type=pathlib.Path,
                      required=True,
                      help='The path to the current patterns schema json file.')
    
    countries_to_schema_subparser.add_argument('--countries-json', 
                       dest='countries_json',
                       type=pathlib.Path, 
                       required=False,
                       help='You can manually download a countries.json file and pass it in here. If not it will use the countries data from the script.' )

    countries_to_schema_subparser.add_argument('--output-patterns-schema-json', 
                      dest='output_patterns_schema_json',
                      type=pathlib.Path,
                      required=False,
                      help='The path to the output patterns schema json file. If not supplied the current-patterns-schema-json will be updated in place.')
       
    countries_to_schema_subparser.set_defaults(func=_run_countries_to_schema)