...
from o3de import cmake, o3de_object, project_properties, utils
...
def disable_gem_in_project(gem_name: str = None,
    ...
    if not enabled_gem_file:
        enabled_gem_file = o3de_object.get_enabled_gem_cmake_file(project_path=project_path)
    if enabled_gem_file.is_file():
        # enabled_gems.cmake does not support gem names with specifiers
        gem_found_in_cmake = gem_name_without_specifier in o3de_object.get_enabled_gems(enabled_gem_file)
...
def add_args(subparsers) -> None:
    ...
    utils.add_verbosity_arg(disable_gem_project_subparser)

    group = disable_gem_project_subparser.add_mutually_exclusive_group(required=True)
    ...
    disable_gem_project_subparser.add_argument('-egf', '--enabled-gem-file', type=pathlib.Path, required=False,
                                      help='The cmake enabled gem file in which gem names are to be removed from.'
                                           'If not specified it will assume ')

    disable_gem_project_subparser.set_defaults(func=_run_disable_gem_in_project)