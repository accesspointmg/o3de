...
if [[ -n "$CMAKE_O3DE_PROJECTS" ]]; then
    CONFIGURE_CMD="${CONFIGURE_CMD} -DLY_PROJECTS='${CMAKE_O3DE_PROJECTS}'"
fi
...
# we will check for an environment variable O3DE_MIN_MEMORY_PER_CORE to restrict the number
...
#
# This is based on the value the total memory (in kB) divided by the O3DE_MIN_MEMORY_PER_CORE 
# (in kB, e.g. 2 GB = 2097152 kB)

if [[ "${O3DE_MIN_MEMORY_PER_CORE:-0}" -gt 0 ]]; then
    MIN_MEMORY_PER_CORE=${O3DE_MIN_MEMORY_PER_CORE}
