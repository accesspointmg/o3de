...
def test_DeleteKey_KeyDoesNotExist_SilentlyFails(mocker):
    ...
    mock_delete_child_keys_and_values = mocker.patch(
        "o3de_test_tools.environment.reg_cleaner._delete_child_keys_and_values")
    mock_delete_child_keys_and_values.side_effect = WindowsError

    o3de_test_tools.environment.reg_cleaner._delete_key("key_1")


def test_DeleteKey_KeyIsEmpty_KeyDeleted(mocker):
    # type: (MockFixture) -> None
    mockwinreg = mocker.patch("o3de_test_tools.environment.reg_cleaner.winreg")
    mock_delete_child_keys_and_values = mocker.patch(
        "o3de_test_tools.environment.reg_cleaner._delete_child_keys_and_values")
...
def test_DeleteChildKeysAndValues_KeyDoesNotExist_RaisesWindowsError(mocker):
    ...
    with pytest.raises(WindowsError):
        o3de_test_tools.environment.reg_cleaner._delete_child_keys_and_values("key_1")
...
def test_DeleteChildKeysAndValues_KeyIsEmpty_ReturnsTrue(mocker):
    ...
    assert o3de_test_tools.environment.reg_cleaner._delete_child_keys_and_values("key_1")
...
def test_DeleteChildKeysAndValues_KeyWithChildrenInExceptionList_ReturnsFalse(mocker):
    ...
    assert not o3de_test_tools.environment.reg_cleaner._delete_child_keys_and_values("key_1", exception_list)
