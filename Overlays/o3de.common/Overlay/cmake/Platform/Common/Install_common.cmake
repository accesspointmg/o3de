...
set(_cmake_Platform_Common_Install_common_cmake ${CMAKE_CURRENT_LIST_DIR})
include(${_cmake_Platform_Common_Install_common_cmake}/../../FileUtil.cmake)

set(O3DE_INSTALL_EXTERNAL_BUILD_DIRS "" CACHE PATH "External build directories to be included in the install process. This allows to package non-monolithic and monolithic.")
unset(normalized_external_build_dirs)
foreach(external_dir ${O3DE_INSTALL_EXTERNAL_BUILD_DIRS})
    cmake_path(ABSOLUTE_PATH external_dir BASE_DIRECTORY ${O3DE_ENGINE_PATH} NORMALIZE)
    list(APPEND normalized_external_build_dirs ${external_dir})
endforeach()
set(O3DE_INSTALL_EXTERNAL_BUILD_DIRS ${normalized_external_build_dirs})

set(CMAKE_INSTALL_MESSAGE NEVER) # Simplify messages to reduce output noise

define_property(TARGET PROPERTY O3DE_INSTALL_GENERATE_RUN_TARGET
...
o3de_set(CMAKE_INSTALL_DEFAULT_COMPONENT_NAME CORE)
if(O3DE_MONOLITHIC_GAME)
    set(O3DE_BUILD_PERMUTATION Monolithic)
else()
    set(O3DE_BUILD_PERMUTATION Default)
endif()
string(TOUPPER ${O3DE_BUILD_PERMUTATION} O3DE_INSTALL_PERMUTATION_COMPONENT)
...
cmake_path(APPEND archive_output_directory "${O3DE_PAL_PLATFORM_NAME}/$<CONFIG>/${O3DE_BUILD_PERMUTATION}")
cmake_path(APPEND library_output_directory "${O3DE_PAL_PLATFORM_NAME}/$<CONFIG>/${O3DE_BUILD_PERMUTATION}")
cmake_path(APPEND runtime_output_directory "${O3DE_PAL_PLATFORM_NAME}/$<CONFIG>/${O3DE_BUILD_PERMUTATION}")

#! o3de_setup_target: Setup the data needed to re-create the cmake target commands for a single target
function(o3de_setup_target OUTPUT_CONFIGURED_TARGET ALIAS_TARGET_NAME absolute_target_source_dir)
    # De-alias target name
    o3de_de_alias_target(${ALIAS_TARGET_NAME} TARGET_NAME)

    # Get the target source directory relative to the O3DE root folder
    o3de_get_engine_relative_source_dir(${absolute_target_source_dir} relative_target_source_dir)

    # All include directories marked PUBLIC or INTERFACE will be installed. We don't use PUBLIC_HEADER because in order to do that
    ...
    if (include_directories)
        ...
        foreach(include_directory ${include_directories})
            ...
            if(include_genex_expr STREQUAL include_directory) # only for cases where there are no generation expressions
                ...
                cmake_path(IS_PREFIX O3DE_ENGINE_PATH ${absolute_target_source_dir} NORMALIZE include_directory_child_of_o3de_root)
                if(NOT include_directory_child_of_o3de_root)
                    # Include directory is outside of the O3DE root folder ${O3DE_ENGINE_PATH}.
                ...
                if(include_directory_child_of_build)
                    ...
                    cmake_path(RELATIVE_PATH include_directory BASE_DIRECTORY ${O3DE_ENGINE_PATH} OUTPUT_VARIABLE rel_include_dir)
                endif()
                cmake_path(APPEND rel_include_dir "..")
                cmake_path(NORMAL_PATH rel_include_dir OUTPUT_VARIABLE destination_dir)

                o3de_install(DIRECTORY ${include_directory}
                    ...
                    FILES_MATCHING
                        ...
                        PATTERN *.inl
                        PATTERN *.hxx
                        PATTERN *.jinja # O3deAutoGen files
                )
            endif()
        endforeach()
    endif()
    ...
    if(COMMAND o3de_setup_target_install_targets_override)
        # Mac needs special handling because of a cmake issue
        o3de_setup_target_install_targets_override(TARGET ${TARGET_NAME}
    ...
    get_target_property(should_create_helper ${TARGET_NAME} O3DE_INSTALL_GENERATE_RUN_TARGET)
    ...
    # Remove the _LIBRARY since we dont need to pass that to o3de_add_targets
    ...
    foreach(include IN LISTS include_directories)
        ...
        if(include_genex_expr STREQUAL include) # only for cases where there are no generation expressions
            ...
            if(include_directory_child_of_build)
                ...
                cmake_path(SET base_path ${O3DE_ENGINE_PATH})
    ...
    string(REPEAT " " 12 PLACEHOLDER_INDENT)
    get_property(interface_build_dependencies_props TARGET ${TARGET_NAME} PROPERTY O3DE_DELAYED_LINK)
    ...
    # We don't want to put such dependencies in the interface because a user can mistakenly use a symbol that is not available
    ...
    if(interface_build_dependencies_props)
        ...
        # dependencies are transferred to the downstream dependencies
    ...
    string(REPEAT " " 8 PLACEHOLDER_INDENT)
    # If a target has an O3DE_PROJECT_NAME property, forward that property to new target
    get_target_property(target_project_association ${TARGET_NAME} O3DE_PROJECT_NAME)
    if(target_project_association)
        list(APPEND TARGET_PROPERTIES_PLACEHOLDER "${PLACEHOLDER_INDENT}O3DE_PROJECT_NAME ${target_project_association}")
    endif()
...
set_target_properties(${RUN_TARGET_NAME} PROPERTIES
    ...
    VS_DEBUGGER_COMMAND_ARGUMENTS \"--project-path=\${O3DE_DEFAULT_PROJECT_PATH}\"
)
set_property(GLOBAL APPEND PROPERTY O3DE_ALL_TARGETS ${RUN_TARGET_NAME})
"
)
    endif()
    ...
    if(NOT target_type STREQUAL INTERFACE_LIBRARY)
        ...
        if(target_type IN_LIST runtime_types)
            set(target_location "\${O3DE_ENGINE_PATH}/${runtime_output_directory}/${target_runtime_output_subdirectory}/$<TARGET_FILE_NAME:${TARGET_NAME}>")
        elseif(target_type STREQUAL MODULE_LIBRARY)
            set(target_location "\${O3DE_ENGINE_PATH}/${library_output_directory}/${target_library_output_subdirectory}/$<TARGET_FILE_NAME:${TARGET_NAME}>")
        elseif(target_type STREQUAL SHARED_LIBRARY)
            string(APPEND target_file_contents
"set_property(TARGET ${NAME_PLACEHOLDER}
    APPEND_STRING PROPERTY IMPORTED_IMPLIB
        $<$<CONFIG:$<CONFIG>$<ANGLE-R>:\"\${O3DE_ENGINE_PATH}/${archive_output_directory}/$<TARGET_LINKER_FILE_NAME:${TARGET_NAME}>\"$<ANGLE-R>
)
")
            string(APPEND target_file_contents
"set_property(TARGET ${NAME_PLACEHOLDER}
    PROPERTY IMPORTED_IMPLIB_$<UPPER_CASE:$<CONFIG>>
        \"\${O3DE_ENGINE_PATH}/${archive_output_directory}/$<TARGET_LINKER_FILE_NAME:${TARGET_NAME}>\"
)
")
            set(target_location "\${O3DE_ENGINE_PATH}/${library_output_directory}/${target_library_output_subdirectory}/$<TARGET_FILE_NAME:${TARGET_NAME}>")
        elseif(target_type STREQUAL STATIC_LIBRARY) # STATIC_LIBRARY, OBJECT_LIBRARY, INTERFACE_LIBRARY
            set(target_location "\${O3DE_ENGINE_PATH}/${archive_output_directory}/$<TARGET_LINKER_FILE_NAME:${TARGET_NAME}>")
...
")
    ...
    file(GENERATE OUTPUT "${target_install_source_dir}/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}/${NAME_PLACEHOLDER}_$<CONFIG>.cmake" CONTENT "${target_file_contents}")

    foreach(conf IN LISTS CMAKE_CONFIGURATION_TYPES)
        string(TOUPPER ${conf} UCONF)
        o3de_install(FILES "${target_install_source_dir}/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}/${NAME_PLACEHOLDER}_${conf}.cmake"
            DESTINATION ${relative_target_source_dir}/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}
            COMPONENT ${O3DE_INSTALL_PERMUTATION_COMPONENT}_${UCONF}
            CONFIGURATIONS ${conf}
        )
    endforeach()
    ...
    o3de_file_read(${O3DE_ENGINE_PATH}/cmake/install/InstalledTarget.in target_cmakelists_template)
...
#! o3de_setup_subdirectories: setups all targets on a per directory basis
function(o3de_setup_subdirectories)
    get_property(all_subdirectories GLOBAL PROPERTY O3DE_ALL_TARGET_DIRECTORIES)
    foreach(target_subdirectory IN LISTS all_subdirectories)
        o3de_setup_subdirectory(${target_subdirectory})
    endforeach()
endfunction()

#! o3de_setup_3p_target: Export enough data such that the script-only mode knows about the runtime dependencies
...
function(o3de_setup_3p_target OUTVAR_GENERATED_DATA_FILENAME OUTVAR_FILE_NAME_TO_GENERATE ALIAS_TARGET_NAME)
    o3de_de_alias_target(${ALIAS_TARGET_NAME} TARGET_NAME)
    ...
    file(GENERATE OUTPUT  "${CMAKE_BINARY_DIR}/install/$<CONFIG>/${O3DE_PAL_PLATFORM_NAME}/${CLEAN_TARGET_NAME}.cmake" 
                  CONTENT "${generate_content}")
    set(${OUTVAR_GENERATED_DATA_FILENAME} "${CMAKE_BINARY_DIR}/install/$<CONFIG>/${O3DE_PAL_PLATFORM_NAME}/${CLEAN_TARGET_NAME}.cmake" PARENT_SCOPE)
...
function(o3de_setup_3p_dependencies)
    ...
    # prevent this code from running in non-installer-builds, otherwise it will try to hang these commands off a non-existent generic
    ...
    set(final_3p_output_dir "${CMAKE_BINARY_DIR}/cmake/3rdParty/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}")
    get_property(o3de_all_3rdparty_targets GLOBAL PROPERTY O3DE_ALL_3RDPARTY_TARGETS)

    # call the o3de_setup_3p_target for each 3p target.  It will create a GENERATE step
    ...
    foreach(target_3p IN LISTS o3de_all_3rdparty_targets)
        o3de_setup_3p_target(file_to_process output_filename ${target_3p})
        if (file_to_process)
    ...
    if (list_of_files_to_process)
        add_custom_command(
            COMMAND ${CMAKE_COMMAND} -P 
                "${O3DE_ENGINE_PATH}/cmake/3rdParty/script-only-mode/PostProcessScriptOnlyMappings.cmake" # Script to run
            ...
            DEPENDS ${list_of_files_to_process} 
                    "${O3DE_ENGINE_PATH}/cmake/3rdParty/script-only-mode/PostProcessScriptOnlyMappings.cmake"
            VERBATIM
        )
        ...
        o3de_install(FILES ${expected_output_files}
            DESTINATION "cmake/3rdParty/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}"
            COMPONENT ${O3DE_INSTALL_PERMUTATION_COMPONENT})
    endif()
endfunction()


#! o3de_setup_subdirectory: setup all targets in the subdirectory
function(o3de_setup_subdirectory absolute_target_source_dir)
    o3de_get_engine_relative_source_dir(${absolute_target_source_dir} relative_target_source_dir)

    # The builtin BUILDSYSTEM_TARGETS property isn't being used here as that returns the de-aliased
    # TARGET and we need the alias namespace for recreating the CMakeLists.txt in the install layout
    get_property(ALIAS_TARGETS_NAME DIRECTORY ${absolute_target_source_dir} PROPERTY O3DE_DIRECTORY_TARGETS)
    foreach(ALIAS_TARGET_NAME IN LISTS ALIAS_TARGETS_NAME)
        o3de_setup_target(configured_target ${ALIAS_TARGET_NAME} ${absolute_target_source_dir})
    ...
    o3de_file_read(${O3DE_ENGINE_PATH}/cmake/install/Copyright.in cmake_copyright_comment)

    # 1. Create the base CMakeLists.txt that will just include a cmake file per platform
    string(CONFIGURE [[
@cmake_copyright_comment@
if(O3DE_MONOLITHIC_GAME)
    include(Platform/${O3DE_PAL_PLATFORM_NAME}/platform_${O3DE_PAL_PLATFORM_WART}.cmake OPTIONAL)
else()
    include(Platform/${O3DE_PAL_PLATFORM_NAME}/platform_${O3DE_PAL_PLATFORM_WART}.cmake)
endif()
    ...
    # In the o3de_setup_assets() function, it generates an empty CMakeLists.txt into the gem root directory
    ...
    o3de_install(FILES "${target_install_source_dir}/CMakeLists.txt"
        DESTINATION ${relative_target_source_dir}
        COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
    )

    # 2. For this platform file, create a Platform/${O3DE_PAL_PLATFORM_NAME}/platform_${O3DE_PAL_PLATFORM_WART}.cmake file
    #    that will include different configuration permutations (e.g. monolithic vs non-monolithic)
    file(CONFIGURE OUTPUT "${target_install_source_dir}/Platform/${O3DE_PAL_PLATFORM_NAME}/platform_${O3DE_PAL_PLATFORM_WART}.cmake" CONTENT [[
@cmake_copyright_comment@
if(O3DE_MONOLITHIC_GAME)
    include(Platform/${O3DE_PAL_PLATFORM_NAME}/Monolithic/permutation.cmake OPTIONAL)
else()
    include(Platform/${O3DE_PAL_PLATFORM_NAME}/Default/permutation.cmake)
endif()
]])
    o3de_install(FILES "${target_install_source_dir}/Platform/${O3DE_PAL_PLATFORM_NAME}/platform_${O3DE_PAL_PLATFORM_WART}.cmake"
        DESTINATION ${relative_target_source_dir}/Platform/${O3DE_PAL_PLATFORM_NAME}
        COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
    )

    # 3. For this configuration permutation, generate a Platform/${O3DE_PAL_PLATFORM_NAME}/${permutation}/permutation.cmake
    #    that will declare the target and configure it
    o3de_setup_subdirectory_create_alias("${absolute_target_source_dir}" CREATE_ALIASES_PLACEHOLDER)
    o3de_setup_subdirectory_set_gem_variant_to_load("${absolute_target_source_dir}" GEM_VARIANT_TO_LOAD_PLACEHOLDER)
    o3de_setup_add_variant_dependencies_for_gem_dependencies("${absolute_target_source_dir}" O3DE_ADD_VARIANT_DEPS_FOR_GEM_DEPS)
    o3de_setup_subdirectory_enable_gems("${absolute_target_source_dir}" ENABLE_GEMS_PLACEHOLDER)
    o3de_setup_subdirectory_install_code("${absolute_target_source_dir}" O3DE_INSTALL_CODE_PLACEHOLDER)

    # Write out all the aggregated o3de_add_target function calls and the final o3de_create_alias() calls to the target CMakeLists.txt
    file(WRITE "${target_install_source_dir}/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}/permutation.cmake"
        "${cmake_copyright_comment}"
    ...
    o3de_install(FILES "${target_install_source_dir}/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}/permutation.cmake"
        DESTINATION ${relative_target_source_dir}/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}
        COMPONENT ${O3DE_INSTALL_PERMUTATION_COMPONENT}
    )
...
#! o3de_setup_cmake_install: install the "cmake" folder
function(o3de_setup_cmake_install)

    o3de_install(DIRECTORY "${O3DE_ENGINE_PATH}/cmake"
        DESTINATION .
        COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
    ...
    o3de_install(FILES "${O3DE_ENGINE_PATH}/cmake/install/ConfigurationTypes.cmake"
        DESTINATION cmake
        COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
    )
    ...
    foreach(conf IN LISTS CMAKE_CONFIGURATION_TYPES)
        string(TOUPPER ${conf} UCONF)
        configure_file("${O3DE_ENGINE_PATH}/cmake/install/ConfigurationType_config.cmake.in"
            "${CMAKE_BINARY_DIR}/cmake/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}/ConfigurationTypes_${conf}.cmake"
            @ONLY
        )
        o3de_install(FILES "${CMAKE_BINARY_DIR}/cmake/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}/ConfigurationTypes_${conf}.cmake"
            DESTINATION cmake/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}
            COMPONENT ${O3DE_INSTALL_PERMUTATION_COMPONENT}_${UCONF}
            CONFIGURATIONS ${conf}
        )
    endforeach()
    ...
    # get_external_subdirectories_in_use was never implemented in this
    # fork — the closest equivalent is the full registered set
    get_all_external_subdirectories(external_subdirs)
    foreach(external_subdir ${external_subdirs})
        ...
        o3de_get_root_subdirectory_which_is_parent(${external_subdir} root_subdir_of_external_subdir)
        ...
        cmake_path(IS_PREFIX O3DE_ENGINE_PATH ${external_subdir} is_subdirectory_of_engine)
    ...
    o3de_read_json_array(engine_templates ${O3DE_ENGINE_PATH}/engine.json "templates")
    if(engine_templates)
    ...
    o3de_read_json_array(engine_repos ${O3DE_ENGINE_PATH}/engine.json "repos")
    if(engine_repos)
    ...
    o3de_read_json_key(O3DE_INSTALL_API_VERSIONS ${O3DE_ENGINE_PATH}/engine.json "api_versions")

    configure_file(${O3DE_ENGINE_PATH}/cmake/install/engine.json.in ${CMAKE_CURRENT_BINARY_DIR}/cmake/engine.json @ONLY)

    o3de_install(FILES
            "${O3DE_ENGINE_PATH}/CMakeLists.txt"
            "${CMAKE_CURRENT_BINARY_DIR}/cmake/engine.json"
        DESTINATION .
        COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
    )

    # Collect all Find files that were added with o3de_add_external_target_path
    ...
    foreach(additional_module_path ${additional_module_paths})
        ...
        foreach(find_file ${find_files})
            ...
            unset(plat_files)
            file(GLOB plat_files "${find_file_parent}/Platform/${O3DE_PAL_PLATFORM_NAME}/*.cmake")
            list(APPEND additional_platform_files "${plat_files}")
        endforeach()
    endforeach()

    o3de_install(FILES ${additional_find_files}
        DESTINATION cmake/3rdParty
        COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
    )
    o3de_install(FILES ${additional_platform_files}
        DESTINATION cmake/3rdParty/Platform/${O3DE_PAL_PLATFORM_NAME}
        COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
    )

    # Findo3de.cmake file: we generate a different Findo3de.cmake file than the one we have in the source dir.
    configure_file(${O3DE_ENGINE_PATH}/cmake/install/Findo3de.cmake.in ${CMAKE_CURRENT_BINARY_DIR}/cmake/Findo3de.cmake @ONLY)
    o3de_install(FILES "${CMAKE_CURRENT_BINARY_DIR}/cmake/Findo3de.cmake"
        DESTINATION cmake
        COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
    )

    unset(find_subdirectories)
    # Add to find_subdirectories all directories in which o3de_add_target were called in
    get_property(all_subdirectories GLOBAL PROPERTY O3DE_ALL_TARGET_DIRECTORIES)
    foreach(target_subdirectory IN LISTS all_subdirectories)
        ...
        get_property(is_external_subdir GLOBAL PROPERTY "O3DE_EXTERNAL_SUBDIRECTORY_${target_subdirectory}" SET)
        if(is_external_subdir)
            continue()
        endif()
        o3de_get_root_subdirectory_which_is_parent(${target_subdirectory} root_subdir_of_target)
        ...
        cmake_path(IS_PREFIX O3DE_ENGINE_PATH ${target_subdirectory} is_subdirectory_of_engine)
        if(NOT is_subdirectory_of_engine)
    ...
    endforeach()
    set(permutation_find_subdirectories ${CMAKE_CURRENT_BINARY_DIR}/cmake/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}/o3de_subdirectories_${O3DE_PAL_PLATFORM_WART}.cmake)
...
"
    ...
    o3de_install(FILES "${permutation_find_subdirectories}"
        DESTINATION cmake/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}
        COMPONENT ${O3DE_INSTALL_PERMUTATION_COMPONENT}
    )

    set(pal_builtin_file ${CMAKE_CURRENT_BINARY_DIR}/cmake/3rdParty/Platform/${O3DE_PAL_PLATFORM_NAME}/BuiltInPackages_${O3DE_PAL_PLATFORM_WART}${O3DE_ARCHITECTURE_NAME_EXTENSION}.cmake)
    file(GENERATE OUTPUT ${pal_builtin_file}
        CONTENT
"# Generated by O3DE install\n
if(O3DE_MONOLITHIC_GAME)
    include(${_cmake_Platform_Common_Install_common_cmake}/../../3rdParty/Platform/${O3DE_PAL_PLATFORM_NAME}/Monolithic/BuiltInPackages_${O3DE_PAL_PLATFORM_WART}${O3DE_ARCHITECTURE_NAME_EXTENSION}.cmake)
else()
    include(${_cmake_Platform_Common_Install_common_cmake}/../../3rdParty/Platform/${O3DE_PAL_PLATFORM_NAME}/Default/BuiltInPackages_${O3DE_PAL_PLATFORM_WART}${O3DE_ARCHITECTURE_NAME_EXTENSION}.cmake)
endif()
"
    )
    o3de_install(FILES "${pal_builtin_file}"
        DESTINATION cmake/3rdParty/Platform/${O3DE_PAL_PLATFORM_NAME}
        COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
    )

    # ${O3DE_BUILD_PERMUTATION}/BuiltInPackage_<platform>.cmake: since associations could happen in any cmake file across the engine. We collect
    # all the associations in o3de_associate_package and then generate them into BuiltInPackages_<platform>.cmake. This will consolidate all
    # associations in one file
    # Associations are sensitive to platform and build permutation, so we make different files for each.
    get_property(all_package_names GLOBAL PROPERTY O3DE_PACKAGE_NAMES)
    ...
    foreach(package_name IN LISTS all_package_names)
        get_property(package_hash GLOBAL PROPERTY O3DE_PACKAGE_HASH_${package_name})
        get_property(targets GLOBAL PROPERTY O3DE_PACKAGE_TARGETS_${package_name})
        list(REMOVE_DUPLICATES targets)
        string(APPEND builtinpackages "o3de_associate_package(PACKAGE_NAME ${package_name} TARGETS ${targets} PACKAGE_HASH ${package_hash})\n")
    endforeach()
    ...
    set(permutation_builtin_file ${CMAKE_CURRENT_BINARY_DIR}/cmake/3rdParty/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}/BuiltInPackages_${O3DE_PAL_PLATFORM_WART}${O3DE_ARCHITECTURE_NAME_EXTENSION}.cmake)
    file(GENERATE OUTPUT ${permutation_builtin_file}
        CONTENT "${builtinpackages}"
    )
    o3de_install(FILES "${permutation_builtin_file}"
        DESTINATION cmake/3rdParty/Platform/${O3DE_PAL_PLATFORM_NAME}/${O3DE_BUILD_PERMUTATION}
        COMPONENT ${O3DE_INSTALL_PERMUTATION_COMPONENT}
    )
...
#! o3de_setup_runtime_dependencies: install runtime dependencies
function(o3de_setup_runtime_dependencies)

    # Common functions used by the below code
    if(COMMAND o3de_setup_runtime_dependencies_copy_function_override)
        ...
        # The template is needed to replace the @O3DE_COPY_PERMISSIONS@ variable with the permissions
        # that should be used for the copied file
        set(install_runtime_o3de_copy_template [[
        function(o3de_copy source_files relative_target_directory)
            ...
            foreach(source_file IN LISTS source_files)
                ...
                if("${source_file}" IS_NEWER_THAN "${target_file}")
                    ...
                    file(COPY "${source_file}" DESTINATION "${full_target_directory}" FILE_PERMISSIONS @O3DE_COPY_PERMISSIONS@ FOLLOW_SYMLINK_CHAIN)
        ...
        string(CONFIGURE "${install_runtime_o3de_copy_template}" install_runtime_o3de_copy @ONLY)
        foreach(conf IN LISTS CMAKE_CONFIGURATION_TYPES)
            string(TOUPPER ${conf} UCONF)
            o3de_install(CODE "${install_runtime_o3de_copy}"
                COMPONENT ${O3DE_INSTALL_PERMUTATION_COMPONENT}_${UCONF}
            )
        endforeach()
    endif()
    ...
    list(JOIN runtime_commands "    " runtime_commands_str) # the spaces are just to see the right indentation in the cmake_install.cmake file
    foreach(conf IN LISTS CMAKE_CONFIGURATION_TYPES)
        string(TOUPPER ${conf} UCONF)
        o3de_install(CODE "${runtime_commands_str}"
            COMPONENT ${O3DE_INSTALL_PERMUTATION_COMPONENT}_${UCONF}
        )
    endforeach()
...
#! o3de_setup_assets: install asset directories required by the engine
function(o3de_setup_assets)
    ...
    get_gems_in_use(gems_non_realpath)
    foreach(gem_candidate_dir IN LISTS gems_non_realpath)
        file(REAL_PATH ${gem_candidate_dir} gem_candidate_dir BASE_DIRECTORY ${O3DE_ENGINE_PATH})
        list(APPEND gems ${gem_candidate_dir})
    endforeach()
    ...
    foreach(gem_candidate_dir IN LISTS gems)
        unset(gem_files)
        unset(gem_files_tenative)
        ...
        # Storing a "mapping" of gem candidate directories, to gem files using
        # a DIRECTORY property for the "value" and the GLOBAL property for the "key"
        set_property(DIRECTORY ${gem_candidate_dir} APPEND PROPERTY directory_filtered_asset_paths "${gem_files}")
    ...
    foreach(gem_candidate_dir IN LISTS gem_candidate_dirs)
        ...
        cmake_path(IS_PREFIX O3DE_ENGINE_PATH ${gem_candidate_dir} is_gem_subdirectory_of_engine)
        ...
        foreach(filtered_asset_path IN LISTS filtered_asset_paths)
            ...
            # Currently only the gem.json and preview.png files are copied over
        ...
        foreach(gem_absolute_path IN LISTS gems_assets_paths)
            # If a gem is not a subdirectory of the engine root, then
            # prepend "External" to its subdirectory root
            o3de_get_root_subdirectory_which_is_parent(${gem_candidate_dir} root_subdir_of_gem)
            ...
            if(IS_DIRECTORY ${gem_absolute_path})
                o3de_install(DIRECTORY "${gem_absolute_path}"
            ...
            elseif (EXISTS ${gem_absolute_path})
                ...
                if (is_gem_root AND NOT has_gem_root_cmakelist_content)
                    ...
                    o3de_install(FILES "${gem_scratch_binary_dir}/CMakeLists.txt"
                        DESTINATION ${gem_install_dest_dir}
                        COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
                    )
                endif()

                o3de_install(FILES ${gem_absolute_path}
                    DESTINATION ${gem_install_dest_dir}
                    COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
                )
            endif()
...
#! o3de_setup_add_variant_dependencies_for_gem_dependencies: Replicates the call to
...
function(o3de_setup_add_variant_dependencies_for_gem_dependencies absolute_target_source_dir output_script)
...
#! o3de_setup_subdirectory_create_alias: Replicates the call to the `o3de_create_alias` function
#! within the generated CMakeLists.txt in the same relative install layout directory
function(o3de_setup_subdirectory_create_alias absolute_target_source_dir output_script)
    # Replicate the create_alias() calls made in the SOURCE_DIR into the generated CMakeLists.txt
    string(JOIN "\n" create_alias_template
        "if(NOT TARGET @alias_name@)"
        "   o3de_create_alias(@create_alias_args@)"
        "endif()"
        "")

    unset(${output_script} PARENT_SCOPE)
    get_property(create_alias_args_list DIRECTORY ${absolute_target_source_dir} PROPERTY O3DE_CREATE_ALIAS_ARGUMENTS)
...
#! o3de_setup_subdirectory_set_gem_variant_to_load: Replicates the call to the `o3de_set_gem_variant_to_load` function
#! within the generated CMakeLists.txt in the same relative install layout directory
function(o3de_setup_subdirectory_set_gem_variant_to_load absolute_target_source_dir output_script)
    # Replicate the o3de_set_gem_variant_to_load() calls made in the SOURCE_DIR for into the generated CMakeLists.txt
    set(set_gem_variant_args_template "o3de_set_gem_variant_to_load(@set_gem_variant_args@)\n")

    unset(${output_script} PARENT_SCOPE)
    get_property(set_gem_variant_args_lists DIRECTORY ${absolute_target_source_dir} PROPERTY O3DE_SET_GEM_VARIANT_TO_LOAD_ARGUMENTS)
...
#! o3de_setup_subdirectory_enable_gems: Replicates the call to the `o3de_enable_gems` function
#! within the generated CMakeLists.txt in the same relative install layout directory
function(o3de_setup_subdirectory_enable_gems absolute_target_source_dir output_script)
    # Replicate the o3de_set_gem_variant_to_load() calls made in the SOURCE_DIR into the generated CMakeLists.txt
    set(enable_gems_template "o3de_enable_gems(@enable_gems_args@)\n")

    unset(${output_script} PARENT_SCOPE)
    get_property(enable_gems_args_list DIRECTORY ${absolute_target_source_dir} PROPERTY O3DE_ENABLE_GEMS_ARGUMENTS)
...
#! o3de_setup_subdirectory_install_code: Add the CMake code specified in the O3DE_SUBDIRECTORY_INSTALL_CODE
#!  DIRECTORY property to the beginning of the generated CMakeLists.txt in the same relative install layout directory
#! within the generated CMakeLists.txt in the same relative install layout directory
function(o3de_setup_subdirectory_install_code absolute_target_source_dir output_script)
    unset(${output_script} PARENT_SCOPE)
...
#! o3de_setup_o3de_install: orchestrates the installation of the different parts. This is the entry point from the root CMakeLists.txt
function(o3de_setup_o3de_install)

    o3de_setup_subdirectories()
    o3de_setup_cmake_install()
    o3de_setup_runtime_dependencies()
    o3de_setup_assets()
    o3de_setup_3p_dependencies()

    # Misc
    o3de_install(FILES
        ${O3DE_ENGINE_PATH}/pytest.ini
        ${O3DE_ENGINE_PATH}/LICENSE.txt
        ${O3DE_ENGINE_PATH}/README.md
        ${O3DE_ENGINE_PATH}/CMakePresets.json
        DESTINATION .
        COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
    )
    ...
    if("$ENV{O3DE_PACKAGE_TYPE}" STREQUAL "DEB")
        o3de_install(FILES
            ${O3DE_ENGINE_PATH}/Code/Tools/ProjectManager/Resources/o3de_desktop.svg
            DESTINATION .
            COMPONENT ${CMAKE_INSTALL_DEFAULT_COMPONENT_NAME}
        )
    endif()
    ...
    foreach(external_dir ${O3DE_INSTALL_EXTERNAL_BUILD_DIRS})
        o3de_install(CODE
"set(O3DE_CORE_COMPONENT_ALREADY_INCLUDED TRUE)
include(${external_dir}/cmake_install.cmake)
set(O3DE_CORE_COMPONENT_ALREADY_INCLUDED FALSE)"
            ALL_COMPONENTS
        )
    endforeach()

    if(COMMAND o3de_post_install_steps)
        o3de_post_install_steps()
    endif()
