...
Generic fixtures bundled with O3deTestTools
...
import o3de_test_tools._internal.pytest_plugin
import o3de_test_tools._internal.log.py_logging_util as py_logging_util
import o3de_test_tools._internal.managers.o3de_process_killer as o3de_process_killer
import o3de_test_tools.builtin.helpers as helpers
import o3de_test_tools.environment.file_system
import o3de_test_tools.launchers.launcher_helper
import o3de_test_tools.launchers.platforms.base
import o3de_test_tools.environment.watchdog
from o3de_test_tools import ALL_PLATFORM_OPTIONS, HOST_OS_PLATFORM
...
def pytest_addoption(parser):
    ...
    parser.addoption("--build-directory", nargs='?', default='',
                     help="An existing CMake binary output directory which contains the lumberyard executables,"
                          "such as: D:/o3de/dev/windows_vs2017/bin/profile/")
...
def pytest_configure(config):
    ...
    """
    o3de_test_tools._internal.pytest_plugin.build_directory = _get_build_directory(config)
    o3de_test_tools._internal.pytest_plugin.output_path = _get_output_path(config)
...
def _get_build_directory(config):
    ...
    if custom_build_directory:
        ...
        if custom_build_directory.endswith('debug'):
            pytest.exit("Python debug modules are not available. O3deTestTools test skipped.", 0)
    else:
        # only warn when unset, allowing non-LyTT tests to still use pytest
        logger.warning(f'Pytest custom argument "--build-directory" was not provided, tests using O3deTestTools will fail')
...
def _get_output_path(config):
    ...
    if custom_output_path:
        ...
        if custom_build_directory: # default into known build folder
            default_output_path = os.path.join(custom_build_directory, 'Testing', 'O3deTestTools')
...
def output_path(request):
    ...
    """
    return o3de_test_tools._internal.pytest_plugin.output_path


@pytest.fixture
def build_directory(request):
    return o3de_test_tools._internal.pytest_plugin.build_directory
...
def editor(request, workspace, crash_log_watchdog):
    # type: (...) -> o3de_test_tools.launchers.platforms.base.Launcher
...
def _editor(request, workspace, launcher_platform):
    """Separate implementation to call directly during unit tests"""
    editor = o3de_test_tools.launchers.launcher_helper.create_editor(workspace, launcher_platform)
...
def _launcher(request, workspace, launcher_platform, level=""):
    ...
    if not level:
        launcher = o3de_test_tools.launchers.launcher_helper.create_game_launcher(
...
def automatic_process_killer(request):
    # type: (_pytest.fixtures.SubRequest) -> o3de_process_killer
    """
    ...
    If no "processes_to_kill" is found, it will default to o3de_process_killer.O3DE_PROCESS_KILL_LIST instead.
    :param request: _pytest.fixtures.SubRequest request object.
    :return: o3de_process_killer module.
    """
    processes_to_kill = get_fixture_argument(request, 'processes_to_kill', o3de_process_killer.O3DE_PROCESS_KILL_LIST)
    return _automatic_process_killer(processes_to_kill)


def _automatic_process_killer(processes_to_kill):
    # type: (list) -> o3de_process_killer
    """
    Separate implementation to call directly during unit tests.
    """
    # Detect processes.
    processes_detected = o3de_process_killer.detect_lumberyard_processes(processes_list=processes_to_kill)

    # Kill processes.
    o3de_process_killer.kill_processes(processes_list=processes_detected)

    return o3de_process_killer
...
def workspace(request,  # type: _pytest.fixtures.SubRequest
              ...
              record_build_name,  # type: o3de_test_tools._internal.pytest_plugin.test_tools_fixtures.record_build_name
              output_path,  # type: o3de_test_tools._internal.pytest_plugin.test_tools_fixtures.output_path
              asset_processor_platform,  # type: str
              ):
    # type: (...) -> o3de_test_tools._internal.managers.workspace.WorkspaceManager
    """
    ...
    :param record_build_name: O3deTestTools record_build_name fixture
    :param output_path: O3deTestTools output_path fixture
...
def crash_log_watchdog(request, workspace):
    # type: (...) -> o3de_test_tools.environment.watchdog.CrashLogWatchdog
...
def _crash_log_watchdog(request, workspace, raise_on_crash):
    ...
    crash_log_watchdog = o3de_test_tools.environment.watchdog.CrashLogWatchdog(
