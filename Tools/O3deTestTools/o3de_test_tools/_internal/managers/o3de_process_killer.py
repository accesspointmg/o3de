...
O3DE_PROCESS_KILL_LIST = o3de_test_tools.environment.process_utils.O3DE_PROCESS_KILL_LIST  # copy to preserve interface
...
def detect_lumberyard_processes(processes_list):
    ...
    for process_to_detect in processes_list:
        ...
        if o3de_test_tools.environment.process_utils.process_exists(name=process_to_detect, ignore_extensions=True):
...
def kill_processes(processes_list):
    ...
    o3de_test_tools.environment.process_utils.kill_processes_named(names=processes_list, ignore_extensions=True)
