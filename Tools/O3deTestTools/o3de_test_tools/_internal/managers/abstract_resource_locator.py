...
def _find_engine_root(initial_path):
    ...
    # Assumes folder structure similar to: engine_root/dev/Tools/.../o3de_test_tools/builtin
    for _ in range(15):
    ...
    raise exceptions.O3deTestToolsFrameworkException(f"Unable to find engine root directory. Verify root file '{root_file}' exists")
...
def _find_project_json(engine_root, project):
    ...
    if not project_json:
        project_json = o3de_test_tools.environment.file_system.find_ancestor_file(
            target_file_name='project.json', start_path=o3de_test_tools._internal.pytest_plugin.build_directory)
...
class AbstractResourceLocator(object):
    ...
    def devices_file():
        ...
        Return the path to the user's devices.ini file. This has OS specific functionality.
            Windows: %USERPROFILE%/o3de_test_tools/devices.ini
            Mac: ~/o3de_test_tools/devices.ini
        :return: Path to <root>/o3de_test_tools/devices.ini
            a.k.a. %USERPROFILE%/o3de_test_tools/devices.ini
        """
