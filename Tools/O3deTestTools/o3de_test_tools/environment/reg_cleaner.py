...
CONST_O3DE_REG = r'SOFTWARE\O3DE\O3DE'
AUTOMATION_EXCEPTION_LIST = [
    os.path.join(CONST_O3DE_REG, r"Identity"),
    os.path.join(CONST_O3DE_REG, r"Settings\DXInstalled"),
    os.path.join(CONST_O3DE_REG, r"Settings\EditorSettingsVersion"),
    os.path.join(CONST_O3DE_REG, r"Settings\EnableSourceControl"),
    os.path.join(CONST_O3DE_REG, r"Settings\RC_EnableSourceControl"),
]
...
def clean_o3de_keys(exception_list=None):
    """
    Convenience function to delete all Lumberyard registry keys

    :param exception_list: list of child keys and values to skip
    """
    _delete_key(CONST_O3DE_REG, exception_list)


def create_o3de_keys():
    """
    Helper function to create Lumberyard registry keys that are essential to automated testing
    """
    target_key = fr'HKEY_CURRENT_USER\{CONST_O3DE_REG}\Settings'
