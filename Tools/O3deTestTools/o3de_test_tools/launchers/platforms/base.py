...
import o3de_test_tools.environment.process_utils
import o3de_test_tools.environment.waiter
...
class Launcher(object):
    def __init__(self, workspace, args):
        # type: (o3de_test_tools._internal.managers.workspace.AbstractWorkspaceManager, List[str]) -> None
        """
        ...
        self.workspace = workspace  # type: o3de_test_tools._internal.managers.workspace.AbstractWorkspaceManager
    ...
    def _config_ini_to_dict(self, config_file):
        ...
        if not os.path.exists(config_file):
            raise o3de_test_tools.launchers.exceptions.SetupError(
                f'Default file path not found: "{user_profile_directory}/o3de_test_tools/devices.ini", '
                f'got path: "{config_file}" instead. '
                f'Please create the following file: "{user_profile_directory}/o3de_test_tools/devices.ini" manually. '
                f'Add device IP/ID info inside each section as well.\n'
                'See ~/engine_root/dev/Tools/O3deTestTools/README.txt for more info.')
    ...
    def setup(self, backupFiles=True, launch_ap=True, configure_settings=True):
        ...
        For testing mobile or console devices, make sure you populate the config file located at:
            ~/o3de_test_tools/devices.ini (a.k.a. %USERPROFILE%/o3de_test_tools/devices.ini)
    ...
    def wait(self, timeout=30):
        ...
        """
        o3de_test_tools.environment.waiter.wait_for(
            lambda: not self.is_alive(),
            exc=o3de_test_tools.launchers.exceptions.WaitTimeoutError(f"Application is unexpectedly still active after "
    ...
    def ensure_stopped(self, timeout=30):
        ...
        try:
            o3de_test_tools.environment.waiter.wait_for(
                lambda: not self.is_alive(),
                exc=o3de_test_tools.launchers.exceptions.TeardownError(f"Application is unexpectedly still active after "
                                                                     f"timeout of {timeout} seconds"),
                timeout=timeout
            )
        except o3de_test_tools.launchers.exceptions.TeardownError:
            self._kill()
    ...
    def get_device_config(self, config_file, device_section, device_key):
        ...
        :param config_file: string representing the file path for the config ini file.
            default is '~/o3de_test_tools/devices.ini'
        ...
        try:
            ...
            problem = o3de_test_tools.launchers.exceptions.SetupError(
