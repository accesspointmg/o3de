...
import o3de_test_tools._internal.managers.workspace as workspace_manager
import o3de_test_tools.launchers.platforms.base as base_launcher
...
def create_game_launcher(workspace, launcher_platform=o3de_test_tools.HOST_OS_PLATFORM, args=None):
    ...
    if launcher_platform in GAME_LAUNCHERS:
        launcher_class = o3de_test_tools.LAUNCHERS.get(launcher_platform)
    else:
        log.warning(f"Using default launcher for '{o3de_test_tools.HOST_OS_PLATFORM}' "
                    f"as no option is available for '{launcher_platform}'")
        launcher_class = o3de_test_tools.LAUNCHERS.get(o3de_test_tools.HOST_OS_PLATFORM)
    return launcher_class(workspace, args)


def create_server_launcher(workspace, launcher_platform=o3de_test_tools.HOST_OS_PLATFORM, args=None):
    # type: (workspace_manager.AbstractWorkspaceManager, str, list[str]) -> base_launcher.Launcher
    """
    ...
    if launcher_platform in SERVER_LAUNCHERS:
        ...
        log.warning(f"Using default dedicated launcher for '{o3de_test_tools.HOST_OS_DEDICATED_SERVER}' "
                    f"as no option is available for '{launcher_platform}'")
        launcher_class = o3de_test_tools.LAUNCHERS.get(o3de_test_tools.HOST_OS_DEDICATED_SERVER)
    return launcher_class(workspace, args)


def create_editor(workspace, launcher_platform=o3de_test_tools.HOST_OS_EDITOR, args=None):
    # type: (workspace_manager.AbstractWorkspaceManager, str, list[str]) -> base_launcher.Launcher
    """
    ...
    if not launcher_class:
        log.warning(f"Using default editor launcher for '{o3de_test_tools.HOST_OS_EDITOR}' "
                    f"as no option is available for '{launcher_platform}'")
        launcher_class = o3de_test_tools.LAUNCHERS.get(o3de_test_tools.HOST_OS_EDITOR)
    return launcher_class(workspace, args)


def create_atom_tools_launcher(workspace, app_file_name, launcher_platform=o3de_test_tools.HOST_OS_ATOM_TOOLS, args=None):
    ...
    launcher_class = o3de_test_tools.LAUNCHERS.get(launcher_platform)
    if not launcher_class:
        log.warning(f"Using default Atom Tools launcher for '{o3de_test_tools.HOST_OS_ATOM_TOOLS}' "
                    f"as no option is available for '{launcher_platform}'")
        launcher_class = o3de_test_tools.LAUNCHERS.get(o3de_test_tools.HOST_OS_ATOM_TOOLS)
