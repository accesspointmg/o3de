...
def check_adb_connection_state():
    ...
    if 'more than one device' in output:
        ...
        raise exceptions.O3deTestToolsFrameworkException("Detected unhandled output from adb get-state: {}".format(output.strip()))
...
def pull_files_to_pc(package_name, logs_path, device=None):
    ...
    try:
        ...
        if 'does not exist' in err.output:
            ...
            raise exceptions.O3deTestToolsFrameworkException from err
...
def push_files_to_device(source, destination, device=None):
    ...
    if 'pushed' not in push_result:
        raise exceptions.O3deTestToolsFrameworkException('[AndroidLauncher] Failed to push file to device: {}!'.format(device))
...
def wait_for_android_device_load(android_device_id, timeout=60):
    ...
    Helps deal with race conditions that may occur when calls are made to the O3DE client before loading is complete.
