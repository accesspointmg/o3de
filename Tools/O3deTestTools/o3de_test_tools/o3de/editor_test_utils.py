...
from o3de_test_tools._internal.exceptions import EditorToolsFrameworkException
...
def kill_all_o3de_processes(include_asset_processor: bool = True) -> None:
    """
...
def retrieve_log_path(run_id: int, workspace: o3de_test_tools._internal.managers.workspace.AbstractWorkspaceManager) -> str:
    """
...
def atom_tools_log_path(
        run_id: int, workspace: o3de_test_tools._internal.managers.workspace.AbstractWorkspaceManager) -> str:
    """
...
def retrieve_crash_output(run_id: int, workspace: o3de_test_tools._internal.managers.workspace.AbstractWorkspaceManager,
...
def cycle_crash_report(run_id: int, workspace: o3de_test_tools._internal.managers.workspace.AbstractWorkspaceManager) -> None:
    """
...
def retrieve_editor_log_content(run_id: int,
                                log_name: str,
                                workspace: o3de_test_tools._internal.managers.workspace.AbstractWorkspaceManager,
                                timeout: int = 10) -> str:
    """
...
def save_failed_asset_joblogs(workspace: o3de_test_tools._internal.managers.workspace.AbstractWorkspaceManager) -> None:
    """
...
def prepare_asset_processor(workspace: o3de_test_tools._internal.managers.workspace.AbstractWorkspaceManager,
    ...
    try:
        ...
        if collected_test_data.asset_processor is None:
            if not process_utils.process_exists("AssetProcessor", ignore_extensions=True):
                kill_all_o3de_processes(include_asset_processor=True)
                ...
                kill_all_o3de_processes(include_asset_processor=False)
