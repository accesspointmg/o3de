...
def get_files_hashsum(path_to_files_dir: str) -> Dict[str, int]:
    ...
    try:
        ...
        logger.error("An error occurred in O3deTestTools when trying to read file.")
...
def safe_subprocess(command: str or List[str], **kwargs: Dict) -> ProcessOutput:
    ...
    try:
        ...
        logger.warning(f'Command "{cmd_string}" failed in O3deTestTools with returncode {e.returncode}, output:\n{e.output}')
...
def processes_with_substring_in_name(substring: str) -> tuple:
    ...
    for p in processes:
        try:
            ...
            logger.info(f"Process {p} was killed in O3deTestTools during processes_with_substring_in_name()!\nError: {e}")
            continue
...
def process_cpu_usage_below(process_name: str, cpu_usage_threshold: float) -> bool:
    ...
    if not len(targeted_processes) > 0:
        raise exceptions.O3deTestToolsFrameworkException(f"No instances of {process_name} were found")
...
def check_for_perforce(error_on_no_perforce=True):
    ...
    try:
        ...
        if error_on_no_perforce:
            logger.error(f"Failed to call {command_list} in O3deTestTools with error {e}")
        return False
...
def get_file_hash(filePath, hashBufferSize = 65536):
    if not os.path.exists(filePath):
        raise exceptions.O3deTestToolsFrameworkException(f"Cannot get file hash, file at path '{filePath}' does not exist.")
