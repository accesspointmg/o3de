...
import o3de_test_tools._internal.exceptions as exceptions
...
class AssetProcessor(object):
    ...
    def wait_for_idle(self, timeout=DEFAULT_TIMEOUT_SECONDS):
        ...
        if not self.process_exists():
            raise exceptions.O3deTestToolsFrameworkException("Asset Processor has crashed or unexpectedly shut down during idle wait.")
        if not result == "idle":
            raise exceptions.O3deTestToolsFrameworkException(f"Did not get idle state from AP, message was instead: {result}")
        return True
    ...
    def next_idle(self):
        ...
        if not self.process_exists():
            raise exceptions.O3deTestToolsFrameworkException("Asset Processor has crashed or unexpectedly shut down during idle request.")
        if not result == "idle":
            raise exceptions.O3deTestToolsFrameworkException(f"Did not get idle state from AP, message was instead: {result}")
    ...
    def send_message(self, message):
        ...
        try:
            ...
            logger.warning(f"Failed in O3deTestTools to send message {message} to AP with error {e}")
        return False
    ...
    def read_message(self, read_timeout=DEFAULT_TIMEOUT_SECONDS):
        ...
        try:
            ...
            logger.warning(f"Failed in O3deTestTools to read message from with error {e}")
            return f"error_{e}"
    ...
    def read_port_from_log(self, port_type):
        ...
        def _get_port_from_log():
            ...
            if len(log.runs):
                try:
                    ...
                    logger.debug(f"Failed in O3deTestTools to read port type {port_type} : {port} from file", exc_info=ex)
            return False
        ...
        err = AssetProcessorError(f"Failed in O3deTestTools to read port type {port_type} from {self._workspace.paths.ap_gui_log()}. "
            ...
            try:
                ...
                if not connect_port or not self.using_temp_workspace():
                    ...
                    try:
                        ...
                        logger.debug(f"Failed in O3deTestTools to read port data for {port_name}: {host}:{new_connect_port}",
    ...
    def stop(self, timeout=60):
        ...
        try:
            ...
            logger.warning(f"Failed in O3deTestTools to send quit request with error {e}, stopping")
            self.terminate()
        ...
        try:
            ...
            logger.warning(f"Timeout in O3deTestTools attempting to quit asset processor after {wait_timeout} seconds, using terminate")
            self.terminate()
            return StopReason.TIMEOUT

        if self.process_exists():
            logger.warning(f"Failed in O3deTestTools to stop process {self.get_pid()} after {wait_timeout} seconds, using terminate")
            self.terminate()
    ...
    def backup_ap_settings(self):
        # type: () -> None
        """
        Backs up the asset processor settings file via the O3deSettings class
    ...
    def restore_ap_settings(self):
        # type: () -> None
        """
        Restores the asset processor settings file via the O3deSettings class
    ...
    def gui_process(self, timeout=DEFAULT_TIMEOUT_SECONDS, fastscan=True, capture_output=False, platforms=None,
        ...
        if not platforms:
            if hasattr(self._workspace, "asset_processor_platform"):
                ...
                ap_target = o3de_test_tools.HOST_OS_PLATFORM
            ap_platform = ASSET_PROCESSOR_PLATFORM_MAP.get(ap_target, o3de_test_tools.HOST_OS_PLATFORM)
        ...
        try:
            ...
            time.sleep(1)
            if self._ap_proc.poll() is not None:
                raise AssetProcessorError(f"AssetProcessor immediately quit with errorcode {self._ap_proc.returncode} in O3deTestTools ")
            ...
            if quitonidle:
                waiter.wait_for(lambda: not self.process_exists(), timeout=timeout,
                                exc=AssetProcessorError(f"Failed in O3deTestTools to quit on idle within {timeout} seconds"))
            ...
            try:
                ...
                logger.exception("Ignoring exception while trying to terminate Asset Processor from O3deTestTools")
            raise exceptions.O3deTestToolsFrameworkException from be  # raise whatever prompted us to clean up
    ...
    def _del_readonly(self, action, name, exc):
        ...
        try:
            ...
            logger.error(f'In O3deTestTools Failed to clean up {name} : {e}')
...
def assetprocessorbatch_check_output(workspace, project=None, platforms=None, extra_params=None, log_info=False,
    ...
    try:
        ...
        if not expect_failure:
            logger.error(f"AssetProcessorBatch returned error {ap_path} to O3deTestTools with error {e}")
        ...
        logger.error(f"File Not Found - Failed to call {ap_path} from O3deTestTools with error {e}")
    if expect_failure:
        raise exceptions.O3deTestToolsFrameworkException("AP failure didn't occur as expected")
