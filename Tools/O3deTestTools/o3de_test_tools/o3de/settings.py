...
O3deSettings provides an API for modifying settings files and creating/restoring backups of them.
"""
...
class O3deSettings(object):
    ...
    def _backup_settings(self, settings_file, backup_path):
        ...
        o3de_test_tools.environment.file_system.create_backup(settings_file, backup_path)
    ...
    def _restore_settings(self, settings_file, backup_path):
        ...
        if not backup_path:
            backup_path = self._temp_path
        o3de_test_tools.environment.file_system.restore_backup(settings_file, backup_path)
...
def _edit_text_settings_file(settings_file, setting, value, comment_char=""):
    ...
    if not os.path.isfile(settings_file):
        raise exceptions.O3deTestToolsFrameworkException(f"Invalid file and/or path {settings_file}.")
