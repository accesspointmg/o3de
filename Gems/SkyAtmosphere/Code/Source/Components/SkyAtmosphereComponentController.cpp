...
#define SKY_VIRTUAL_PROPERTY(_Property, _Param) \
    ...
    void SkyAtmosphereComponentController::Reflect(AZ::ReflectContext* context)
    {
    ...
    void SkyAtmosphereComponentController::UpdateSkyAtmosphereParams(SkyAtmosphereParams& params)
    {
        ...
        if (m_configuration.m_sun != m_sunEntityId)
        {
            ...
            if (m_configuration.m_sun.IsValid() && m_configuration.m_sun != m_entityId)
            {
                ...
                if (auto handler = AZ::TransformBus::FindFirstHandler(m_configuration.m_sun); handler == nullptr)
                {
        ...
        const AZ::Transform& transform = m_transformInterface ? m_transformInterface->GetWorldTM() : AZ::Transform::Identity();
        auto sunTransformInterface = AZ::TransformBus::FindFirstHandler(m_configuration.m_sun);
        ...
        if (auto directionalLightInterface = AZ::Render::DirectionalLightRequestBus::FindFirstHandler(m_configuration.m_sun);
        ...
        else
        {
            ...
            AZ::Render::DirectionalLightRequestBus::BroadcastResult(params.m_sunShadowsFarClip, &AZ::Render::DirectionalLightRequests::GetShadowFarClipDistance);
        }
    ...
    void SkyAtmosphereComponentController::Activate(AZ::EntityId entityId)
    {
        ...
        m_featureProcessorInterface = AZ::RPI::Scene::GetFeatureProcessorForEntity<SkyAtmosphereFeatureProcessorInterface>(entityId);
    ...
    void SkyAtmosphereComponentController::Deactivate()
    {
        ...
        AZ::TransformNotificationBus::MultiHandler::BusDisconnect(m_entityId);
        AZ::TransformNotificationBus::MultiHandler::BusDisconnect(m_configuration.m_sun);
    ...
    void SkyAtmosphereComponentController::SetSunDirection(const AZ::Vector3& sunDirection)
    {
        ...
        else if (m_transformInterface)
        {
            ...
            const AZ::Vector3 right = sunDirection.Cross(up);
